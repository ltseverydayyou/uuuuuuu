...
local P = __lt.cs("Players", cloneref);
local T = __lt.cs("TweenService", cloneref);
local M = __lt.cs("MarketplaceService", cloneref);
local U = __lt.cs("UserInputService", cloneref);
...
if not __NAProtectUI(sg, { keepName = false }) then
	sg.Parent = pg;
end;
local cons, alive = {}, true;
local function on(sig, fn)
	if not alive or not sig then
		return nil;
	end;
...
local function gone()
	return not alive or not sg or sg.Parent == nil;
end;
local function NAdrag(ui, dragui)
	...
	on(dragui.InputBegan, function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
			dragging = true;
			dragStart = i.Position;
			startPos = ui.Position;
			on(i.Changed, function()
	...
	on(dragui.InputChanged, function(i)
		if i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch then
			dragInput = i;
		end;
	end);
	on(U.InputChanged, function(i)
...
on(ly:GetPropertyChangedSignal("AbsoluteContentSize"), function()
	if gone() then
		return;
	end;
...
local function bindH(h)
	on(h.Running, function(s)
		if s > 0 then
			stopCur();
		end;
	end);
	on(h.Jumping, stopCur);
	on(h.StateChanged, function(_, ns)
...
if pl.Character then
	bindH(pl.Character:WaitForChild("Humanoid"));
end;
on(pl.CharacterAdded, function(c)
	bindH(c:WaitForChild("Humanoid"));
end);
NAdrag(f, dragBtn);
on(tb.MouseButton1Click, function()
	if gone() then
		return;
	end;
	exp = not exp;
	...
	(__lt.cm("TweenService", "Create", sf, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
		Size = exp and UDim2.new(1, 0, 0, H) or UDim2.new(1, 0, 0, 0)
	})):Play();
	(__lt.cm("TweenService", "Create", f, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
		Size = exp and UDim2.new(0, 260, 0, 60 + H) or UDim2.new(0, 260, 0, 60)
	})):Play();
	if not exp then
		of.Visible = false;
		(__lt.cm("TweenService", "Create", of, TweenInfo.new(0.2), {
			Size = UDim2.new(0, 0, 0, 0)
		})):Play();
	end;
end);
on(cancelBtn.MouseButton1Click, function()
	if gone() then
		return;
	end;
	if pl.Character then
		local h = pl.Character:FindFirstChildOfClass("Humanoid");
		for _, t in ipairs(h and h:GetPlayingAnimationTracks() or {}) do
			t:Stop();
		end;
	end;
	if exp then
		exp = false;
		tb.Text = "Animations ▼";
		(__lt.cm("TweenService", "Create", sf, TweenInfo.new(0.3), {
			Size = UDim2.new(1, 0, 0, 0)
		})):Play();
		(__lt.cm("TweenService", "Create", f, TweenInfo.new(0.3), {
...
on(closeBtn.MouseButton1Click, kill);
on(playBtn.MouseButton1Click, function()
	if gone() then
		return;
	end;
...
on(copyBtn.MouseButton1Click, function()
	if gone() then
		return;
	end;
	if sel then
...
task.spawn(function()
	local D = game:QueryDescendants("Instance");
	local tot = #D;
	for i, inst in ipairs(D) do
		...
		if aid and (not idSet[aid]) then
			...
			if inst:IsA("Animation") then
				...
				local ok, info = pcall(function()
					return __lt.cm("MarketplaceService", "GetProductInfo", aid, Enum.InfoType.Asset);
				end);
		...
		if ld and ld.Parent then
			ld.Text = "Loading... " .. math.floor(i / math.max(tot, 1) * 100) .. "%";
		end;
	end;
	if gone() then
		return;
	...
	for i, d in ipairs(anims) do
		...
		on(b.MouseButton1Click, function()
			...
			sel = d;
			of.Visible = true;
			(__lt.cm("TweenService", "Create", of, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
