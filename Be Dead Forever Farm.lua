...
local function NAdragSmooth(ui, dragui)
    ...
    local function smoothTo(pos)
        ...
        dragTween = TweenService:Create(ui, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position = pos})
        dragTween:Play()
    end
...
bind(title.MouseEnter:Connect(function()
    TweenService:Create(titleScale, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 1.02}):Play()
end))
bind(title.MouseLeave:Connect(function()
    TweenService:Create(titleScale, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 1}):Play()
end))
...
local function headerBtn(txt)
    ...
    bind(b.MouseEnter:Connect(function()
        TweenService:Create(s, TweenInfo.new(0.12, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 1.05}):Play()
    end))
    bind(b.MouseLeave:Connect(function()
        TweenService:Create(s, TweenInfo.new(0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 1}):Play()
    end))
    return b
end
...
local function ripple(target)
    ...
    rc.Parent = r
    TweenService:Create(r, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Size = UDim2.fromOffset(target.AbsoluteSize.X+10, target.AbsoluteSize.Y+10), BackgroundTransparency = 1}):Play()
    task.delay(0.42, function() r:Destroy() end)
end
...
local function toggleRow(text)
    ...
    bind(row.MouseEnter:Connect(function()
        TweenService:Create(rowScale, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 1.02}):Play()
    end))
    bind(row.MouseLeave:Connect(function()
        TweenService:Create(rowScale, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Scale = 1}):Play()
    end))
    ...
    local function setState(on)
        ...
        TweenService:Create(knob, ti, {Position = UDim2.new(0, destX, 0.5, -13)}):Play()
        TweenService:Create(knobScale, TweenInfo.new(0.12, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1.08}):Play()
        task.delay(0.12, function()
            TweenService:Create(knobScale, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Scale = 1}):Play()
        end)
...
local function openUI()
    ...
    header.Visible = true
    body.Visible = true
    TweenService:Create(rootCorner, TweenInfo.new(0.28, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {CornerRadius = UDim.new(0, 14)}):Play()
    TweenService:Create(root, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.fromOffset(openSize.X, openSize.Y)}):Play()
end
...
local function minimizeUI()
    ...
    header.Visible = false
    body.Visible = false
    TweenService:Create(rootCorner, TweenInfo.new(0.26, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {CornerRadius = UDim.new(1, 0)}):Play()
    TweenService:Create(root, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.fromOffset(miniSize.X, miniSize.Y)}):Play()
    task.delay(0.08, function() launcherIcon.Visible = true end)
end
...
bind(closeBtn.MouseButton1Click:Connect(function()
    ...
    monitorCds = {}
    trashCds = {}
    TweenService:Create(root, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 1, Size = UDim2.fromOffset(0,0)}):Play()
