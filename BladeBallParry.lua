...
local targetedSince = {};
do
	local canHook = type(hookmetamethod) == "function" and type(getnamecallmethod) == "function" and type(checkcaller) == "function";
	if canHook and (not G.__parryHook) then
		G.__parryHook = true;
		local old;
		old = hookmetamethod(game, "__namecall", function(self, ...)
			local m = getnamecallmethod();
			if not checkcaller() and m and (m:lower() == "fireserver" or m:lower() == "invokeserver") then
				local a = {
					...
				};
				for i = 1, #a do
					if a[i] == "lufQP9" then
						G.__parryRemote = self;
...
local function buildParryArgs()
	local t = {};
	local alive = workspace:FindFirstChild("Alive");
	if alive then
		for _, ch in ipairs(alive:GetChildren()) do
			local hrp = ch:FindFirstChild("HumanoidRootPart");
			if hrp then
				local x, y, z = cam:WorldToScreenPoint(hrp.Position);
				t[ch.Name] = Vector3.new(x, y, z);
			end;
		end;
	end;
...
local function FireParry()
	local rem = G.__parryRemote
	if rem then
		local args = buildParryArgs()
