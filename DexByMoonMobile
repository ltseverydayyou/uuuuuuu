...
local DexState = (function()
	local stateKey = "__DEXPP_STATE";
	local legacyKey = "__DEXPP";
	local stores = {};
	local seen = {};
	...
	api.Sync = function(state)
		for _, store in ipairs(stores) do
			rawset(store, stateKey, state);
			rawset(store, legacyKey, state);
		end;
		return state;
	end;

	api.Get = function(create)
		for _, store in ipairs(stores) do
			local state = rawget(store, stateKey);
			if type(state) == "table" then
				return api.Sync(state);
			end;

			local legacyState = rawget(store, legacyKey);
			if type(legacyState) == "table" then
				return api.Sync(legacyState);
			end;
		end;
	...
	api.Clear = function(state)
		for _, store in ipairs(stores) do
			if rawget(store, stateKey) == state then
				rawset(store, stateKey, nil);
			end;
			if rawget(store, legacyKey) == state then
				rawset(store, legacyKey, nil);
			end;
		end;
	end;

	api.IsLive = function(state)
		if type(state) ~= "table" then
			return false;
		end;

		local main = rawget(state, "Main");
		if type(main) ~= "table" or type(main.Exit) ~= "function" then
			return false;
		end;
		...
		if typeof(gui) == "Instance" then
			...
			if okParent and parent then
				local okOpenButton, openButton = pcall(function()
					return gui:FindFirstChild("OpenButton");
				end);
				if okOpenButton and openButton then
					return true;
				end;
			end;
		end;
		...
		if rawget(main, "__DexInitStarted") == true then
			...
			if startedAt and (os.clock() - startedAt) <= 30 then
				return true;
			end;
		end;
...
do
	local existingState = DexState.Get(false);
	if existingState and DexState.IsLive(existingState) then
		pcall(function()
...
local EmbeddedModules = {
	Explorer = function()
		...
		local function main()
			...
			local lastContextOpenTime, lastContextOpenPos = 0, nil;
			...
			Explorer.NewListEntry = function(index)
				...
				newEntry.InputBegan:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
						...
						releaseEvent = Main.TrackConn(service.UserInputService.InputEnded:Connect(function(input)
							if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
								releaseEvent:Disconnect();
								mouseEvent:Disconnect();
							end;
						end));
						mouseEvent = Main.TrackConn(service.UserInputService.InputChanged:Connect(function(input)
							if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
			...
			Explorer.Refresh = function()
				...
				for i = 1, maxNodes do
					...
					if node then
						...
						elseif Lib.CheckMouseInGui(entry) then
			...
			local function ShortcutMatchesInput(input, shortcut)
				...
				local ctrlDown = Lib.IsCtrlDown();
				local shiftDown = Lib.IsShiftDown();
				local altDown = IsAltDown();
				return ctrlDown == parsed.Ctrl and shiftDown == parsed.Shift and altDown == parsed.Alt;
			end;
			...
			Explorer.ShowRightClick = function(MousePos)
				...
				if lastContextOpenPos and tick() - lastContextOpenTime < 0.2 and math.abs(lastContextOpenPos.X - mouseX) <= 6 and math.abs(lastContextOpenPos.Y - mouseY) <= 6 then
					return;
				end;
				lastContextOpenTime = tick();
				lastContextOpenPos = Vector2.new(mouseX, mouseY);
				local context = Explorer.RightClickContext;
				local absoluteSize = context.Gui.AbsoluteSize;
				context.MaxHeight = absoluteSize.Y <= 600 and absoluteSize.Y - 40 or nil;
				Explorer.PopulateRightClickContext(context);
				Explorer.LastRightClickX, Explorer.LastRightClickY = mouseX, mouseY;
				context:Show(mouseX, mouseY);
			end;
			Explorer.InitRightClick = function()
				...
				context:Register("TELEPORT_TO", {
					...
					OnClick = function()
						...
						local offset = Settings and Settings.Explorer and Settings.Explorer.TeleportToOffset;
				...
				context:Register("GET_REFERENCES", {
				...
				shortcutInputCon = Main.TrackConn(service.UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
			...
			do
				...
				local classFilterValueCache = {};
				local function ResolveClassFilterValue(str)
					local rawValue = tostring(str or "");
					local cached = classFilterValueCache[rawValue];
					if cached then
						return cached.ClassQuery, cached.NameQuery;
					end;
					local trimmedValue = rawValue:match("^%s*(.-)%s*$") or "";
					local classQuery = trimmedValue:match("^(%S+)") or "";
					local nameQuery = trimmedValue:match("^%S+%s+(.+)$") or "";
					nameQuery = (nameQuery:match("^%s*(.-)%s*$") or ""):lower();
					classFilterValueCache[rawValue] = {
						ClassQuery = classQuery,
						NameQuery = nameQuery
					};
					return classQuery, nameQuery;
				end;
				local function NameMatchesQuery(obj, nameQuery)
					return #nameQuery == 0 or obj.Name:lower():find(nameQuery, 1, true);
				end;
				local function ResolveClassName(str, exactMatch, fallbackToQuery)
					local query = tostring(str or ""):lower();
					if #query == 0 then
						return nil;
					end;
					local cacheKey = (exactMatch and "exact:" or "partial:") .. query;
					local cached = classNameCache[cacheKey];
					if cached ~= nil then
						return cached == false and (fallbackToQuery and str or nil) or cached;
					end;
					local classes = API and API.Classes;
					local partialMatch;
					if type(classes) == "table" then
						for className in pairs(classes) do
							local lowerClassName = className:lower();
							if lowerClassName == query then
								classNameCache[cacheKey] = className;
								return className;
							elseif (not exactMatch) and lowerClassName:find(query, 1, true) then
								partialMatch = className;
							end;
						end;
					end;
					classNameCache[cacheKey] = partialMatch or false;
					return partialMatch or (fallbackToQuery and str or nil);
				end;
				local Only = {
					remotes = {
						"RemoteEvent",
				...
				NewFilter({
					...
					local classQuery, nameQuery = ResolveClassFilterValue(str);
					local className = ResolveClassName(classQuery, false, true);
					return (Obj.ClassName:lower()):find((className or classQuery):lower(), 1, true) and NameMatchesQuery(Obj, nameQuery);
				end);
				NewFilter({
					"classname"
				}, function(Obj, str)
					local classQuery, nameQuery = ResolveClassFilterValue(str);
					local className = ResolveClassName(classQuery, true, false);
					return className and Obj.ClassName == className and NameMatchesQuery(Obj, nameQuery);
				end);
				NewFilter({
					"isa",
					"i",
					...
					local classQuery, nameQuery = ResolveClassFilterValue(str);
					local className = ResolveClassName(classQuery, false, false);
					return className and Obj:IsA(className) and NameMatchesQuery(Obj, nameQuery);
				end);
				NewFilter({
					"only",
					"o"
				...
				entryTemplate = create({
					...
					},
					{
						...
						"ImageLabel",
						{
							BackgroundColor3 = Color3.new(1, 1, 1),
							BackgroundTransparency = 1,
							Image = Main.ResolveAsset("rbxassetid://5642383285"),
				...
				local explorerItems = create({
					...
					},
					{
						...
						"ImageLabel",
						{
							BackgroundColor3 = Color3.new(1, 1, 1),
							BackgroundTransparency = 1,
							Image = Main.ResolveAsset("rbxassetid://5034718129"),
					...
					},
					{
						...
						"ImageLabel",
						{
							BackgroundColor3 = Color3.new(1, 1, 1),
							BackgroundTransparency = 1,
							Image = Main.ResolveAsset("rbxassetid://5642310344"),
							Parent = {
								9
							},
		...
		local function main()
			...
			Properties.NewPropEntry = function(index)
				...
				valueFrame.SoundPreview.InputBegan:Connect(function(input)
					...
					update(input);
					inputEvent = Main.TrackConn(service.UserInputService.InputChanged:Connect(function(input)
						if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
							update(input);
						end;
					end));
			...
			Properties.SetInputProp = function(prop, entryIndex, special)
				...
				elseif Properties.IsTextEditable(prop) then
					inputTextBox.Text = propVal and Properties.ValueToString(prop, propVal) or "";
				...
				Properties.EntryTemplate = create({
					...
					},
					{
						...
						"ImageLabel",
						{
							BackgroundColor3 = Color3.new(1, 1, 1),
							BackgroundTransparency = 1,
							Image = Main.ResolveAsset("rbxassetid://5034718180"),
	...
	ScriptViewer = function()
		...
		local function main()
			local ScriptViewer = {};
			local window, codeFrame, rawSourceBox;
			local PreviousScr = nil;
			local lastSource = "";
			local useRawView = false;
			local function sanitizeSource(source)
				source = tostring(source or "");
				source = source:gsub("\r\n", "\n"):gsub("\r", "\n");
				source = source:gsub("\t", "    ");
				source = source:gsub("\000", "\\0");
				source = source:gsub("[\001-\008\011\012\014-\031\127]", function(ch)
					return ("\\x%02X"):format(string.byte(ch));
				end);
				return source;
			end;
			local luaExpertDecompile = (function()
				if not env or type(env.getscriptbytecode) ~= "function" or type(env.request) ~= "function" then
					return nil;
				end;
				local lastCall = 0;
				local function base64Encode(data)
					if env and type(env.base64encode) == "function" then
						local ok, encoded = pcall(env.base64encode, data);
				...
				return function(scr)
					local okBytecode, bytecode = pcall(env.getscriptbytecode, scr);
					if not okBytecode then
						return nil, "lua.expert failed to get bytecode: " .. tostring(bytecode);
					end;
					if type(bytecode) ~= "string" or bytecode == "" then
						return nil, "lua.expert bytecode is empty.";
					end;
					local timeElapsed = os.clock() - lastCall;
					if timeElapsed < 0.6 then
						task.wait(0.6 - timeElapsed);
					end;
					local okPayload, payload = pcall(service.HttpService.JSONEncode, service.HttpService, {
						script = base64Encode(bytecode)
					});
					if not okPayload then
						return nil, "lua.expert failed to encode request body: " .. tostring(payload);
					end;
					...
					if type(response) ~= "table" then
						return nil, "lua.expert returned an invalid HTTP response.";
					end;
					if response.StatusCode and response.StatusCode ~= 200 then
						return nil, "lua.expert HTTP error (" .. tostring(response.StatusCode) .. "): " .. tostring(response.Body);
					end;
					if type(response.Body) ~= "string" or response.Body == "" then
						return nil, "lua.expert returned an empty response body.";
					end;
					return response.Body;
				end;
			end)();
			local function getDecompiler()
				if Settings and Settings.Decompiler and Settings.Decompiler.UseLuaExpert == true then
					return luaExpertDecompile or function()
						return nil, "lua.expert requires request and getscriptbytecode.";
					end;
				end;
				if env and type(env.decompile) == "function" then
					return env.decompile;
				end;
			...
			local function setSourceText(source)
				lastSource = sanitizeSource(source);
				if codeFrame then
					codeFrame:SetText(lastSource);
				end;
				if rawSourceBox then
					rawSourceBox.Text = lastSource;
				end;
			end;
			local function setRawView(enabled)
				useRawView = enabled and true or false;
				if rawSourceBox then
					rawSourceBox.Visible = useRawView;
				end;
				if codeFrame and codeFrame.Frame then
					codeFrame.Frame.Visible = not useRawView;
				end;
			end;
			local function getRawSelection(textBox)
				if not textBox then
					return nil;
				end;
				local cursorPosition = tonumber(textBox.CursorPosition) or -1;
				local selectionStart = tonumber(textBox.SelectionStart) or -1;
				if cursorPosition < 1 or selectionStart < 1 or cursorPosition == selectionStart then
					return nil;
				end;
				local left = math.min(cursorPosition, selectionStart);
				local right = math.max(cursorPosition, selectionStart);
				if right <= left then
					return nil;
				end;
				return textBox.Text:sub(left, right - 1);
			end;
			ScriptViewer.ViewScript = function(scr)
				local decompileFn = getDecompiler();
				local success, source, decompileErr = false, nil, nil;
				if decompileFn then
					success, source, decompileErr = pcall(decompileFn, scr);
				end;
				if not success or type(source) ~= "string" or source == "" then
					local reason;
					if not decompileFn then
						reason = "No decompiler function found.";
					elseif not success then
						reason = tostring(source);
					else
						reason = tostring(decompileErr or "Decompiler returned empty output.");
					end;
			...
			ScriptViewer.Init = function()
				...
				rawSourceBox = Instance.new("TextBox", window.GuiElems.Content);
				rawSourceBox.BackgroundColor3 = Settings and Settings.Theme and Settings.Theme.Syntax and Settings.Theme.Syntax.Background or Color3.fromRGB(36, 36, 36);
				rawSourceBox.TextColor3 = Settings and Settings.Theme and Settings.Theme.Syntax and Settings.Theme.Syntax.Text or Color3.fromRGB(204, 204, 204);
				rawSourceBox.BorderSizePixel = 0;
				rawSourceBox.Position = UDim2.new(0, 0, 0, 20);
				rawSourceBox.Size = UDim2.new(1, 0, 1, -20);
				rawSourceBox.Font = Enum.Font.Code;
				rawSourceBox.TextSize = 15;
				rawSourceBox.TextXAlignment = Enum.TextXAlignment.Left;
				rawSourceBox.TextYAlignment = Enum.TextYAlignment.Top;
				rawSourceBox.TextWrapped = false;
				rawSourceBox.MultiLine = true;
				rawSourceBox.ClearTextOnFocus = false;
				pcall(function()
					rawSourceBox.RichText = false;
				end);
				rawSourceBox.Text = "";
				rawSourceBox.Visible = false;
				...
				copy.MouseButton1Click:Connect(function()
					...
					local source = useRawView and rawSourceBox.Text or codeFrame:GetText();
					env.setclipboard(source);
				end);
				local copySelection = Instance.new("TextButton", window.GuiElems.Content);
				copySelection.BackgroundTransparency = 1;
				copySelection.Position = UDim2.new(0.2, 0, 0, 0);
				copySelection.Size = UDim2.new(0.2, 0, 0, 20);
				copySelection.Text = "Copy Sel";
				copySelection.TextColor3 = Color3.new(1, 1, 1);
				copySelection.MouseButton1Click:Connect(function()
					if not env.setclipboard then
						return;
					end;
					local selected = "";
					if useRawView then
						selected = getRawSelection(rawSourceBox) or "";
					else
					...
					if selected == "" then
						selected = useRawView and rawSourceBox.Text or codeFrame:GetText();
					end;
					env.setclipboard(selected);
				end);
				local save = Instance.new("TextButton", window.GuiElems.Content);
				save.BackgroundTransparency = 1;
				save.Position = UDim2.new(0.4, 0, 0, 0);
				...
				local viewMode = Instance.new("TextButton", window.GuiElems.Content);
				viewMode.BackgroundTransparency = 1;
				viewMode.Position = UDim2.new(0.6, 0, 0, 0);
				viewMode.Size = UDim2.new(0.2, 0, 0, 20);
				viewMode.Text = "Select Text";
				viewMode.TextColor3 = Color3.new(1, 1, 1);
				viewMode.MouseButton1Click:Connect(function()
					setRawView(not useRawView);
					viewMode.Text = useRawView and "Syntax View" or "Select Text";
					if useRawView and rawSourceBox then
						pcall(function()
							rawSourceBox:CaptureFocus();
							rawSourceBox.CursorPosition = 1;
						end);
					end;
				end);
				local dumpbtn = Instance.new("TextButton", window.GuiElems.Content);
				dumpbtn.BackgroundTransparency = 1;
				dumpbtn.Position = UDim2.new(0.8, 0, 0, 0);
	...
	Lib = function()
		...
		local function main()
			...
			Lib.FormatLuaString = (function()
				...
				return function(str)
					return gsub(str, "[\"\\\000-\031\127-\255]", cleanTable);
				end;
			end)();
			...
			Lib.ShowGui = function(gui)
				if Main and Main.TrackGui then
					Main.TrackGui(gui);
				end;
				if env.gethui then
			...
			Lib.IconMap = (function()
				...
				funcs.DisplayExplorerIcons = function(self, Frame, index)
					if Frame:FindFirstChild("IconMap") then
						...
						obj.BackgroundTransparency = 1;
						obj.Image = Main.ResolveAsset("http://www.roblox.com/asset/?id=" .. self.ExplorerIcons.MapId);
						obj.Name = "IconMap";
			...
			Lib.Window = (function()
				...
				local function getWindowTransparency()
					local transparency = tonumber(Settings and Settings.Window and Settings.Window.Transparency);
				...
				local createGui = function(self)
					local gui = create({
						...
						},
						{
							...
							{
								BackgroundColor3 = Color3.new(0.17647059261799, 0.17647059261799, 0.17647059261799),
								BackgroundTransparency = getWindowTransparency(),
								BorderSizePixel = 0,
								Name = "Content",
								Parent = {
									2
						...
						},
						{
							...
							"ImageLabel",
							{
								BackgroundColor3 = Color3.new(1, 1, 1),
								BackgroundTransparency = 1,
								Image = Main.ResolveAsset("rbxassetid://5054663650"),
								Parent = {
									7
								},
								Position = UDim2.new(0, 3, 0, 3),
						...
						},
						{
							...
							"ImageLabel",
							{
								BackgroundColor3 = Color3.new(1, 1, 1),
								BackgroundTransparency = 1,
								Image = Main.ResolveAsset("rbxassetid://5034768003"),
								Parent = {
									10
								},
								Position = UDim2.new(0, 3, 0, 3),
						...
						},
						{
							...
							"ImageLabel",
							{
								...
								BackgroundTransparency = 1,
								BorderSizePixel = 0,
								Image = Main.ResolveAsset("rbxassetid://1427967925"),
								Name = "Outlines",
								Parent = {
									2
								},
				...
				funcs.ApplyTheme = function(self)
					...
					local themeSettings = Settings and Settings.Theme;
					if not guiElems or not themeSettings then
						return;
					end;
					if guiElems.TopBar then
						guiElems.TopBar.BackgroundColor3 = themeSettings.Main2 or guiElems.TopBar.BackgroundColor3;
					end;
					if guiElems.Content then
						guiElems.Content.BackgroundColor3 = themeSettings.Main1 or guiElems.Content.BackgroundColor3;
						guiElems.Content.BackgroundTransparency = getWindowTransparency();
					end;
					if guiElems.Line then
						guiElems.Line.BackgroundColor3 = themeSettings.Outline1 or guiElems.Line.BackgroundColor3;
					end;
					if guiElems.Title then
						guiElems.Title.TextColor3 = themeSettings.Text or guiElems.Title.TextColor3;
					end;
				end;
				...
				static.ShowWindow = function(window, data)
					...
					elseif align == nil and window.ClosedSide then
				...
				Lib.RefreshTheme = function()
					local winTransparency = getWindowTransparency();
					local themeSettings = Settings and Settings.Theme;
					if leftSide and leftSide.Frame then
						leftSide.Frame.BackgroundTransparency = winTransparency;
					end;
					if rightSide and rightSide.Frame then
						rightSide.Frame.BackgroundTransparency = winTransparency;
					end;
					if themeSettings then
						if leftSide and leftSide.Frame then
							leftSide.Frame.BackgroundColor3 = themeSettings.Main1 or leftSide.Frame.BackgroundColor3;
						end;
						if rightSide and rightSide.Frame then
							rightSide.Frame.BackgroundColor3 = themeSettings.Main1 or rightSide.Frame.BackgroundColor3;
						end;
					end;
					for i = 1, #visibleWindows do
						local window = visibleWindows[i];
						if window and window.ApplyTheme then
							pcall(window.ApplyTheme, window);
						end;
					end;
					local sideWindows = {
						leftSide and leftSide.Windows or nil,
						rightSide and rightSide.Windows or nil
					};
					for _, windowList in ipairs(sideWindows) do
						if windowList then
			...
			Lib.ContextMenu = (function()
				...
				funcs.AddRegistered = function(self, name, disabled)
					...
					local registeredEntry = self.Registered[name];
					if not registeredEntry then
					...
					registeredEntry.Disabled = disabled;
					self.Items[(#self.Items) + 1] = registeredEntry;
					self.Updated = nil;
				end;
				...
				funcs.Show = function(self, x, y)
					...
					self.CloseEvent = Main.TrackConn(service.UserInputService.InputBegan:Connect(function(input)
			...
			Lib.CodeFrame = (function()
				...
				local function setupMouseSelection(obj)
					...
					codeFrame.InputBegan:Connect(function(input)
						if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
							...
							scrollEvent = Main.TrackConn(service.RunService.RenderStepped:Connect(function()
				...
				funcs.ConnectEditBoxEvent = function(self)
					...
					self.EditBoxEvent = Main.TrackConn(service.UserInputService.InputBegan:Connect(function(input)
						...
						local function setupMove(key, func)
							...
							endCon = Main.TrackConn(service.UserInputService.InputEnded:Connect(function(input)
					...
					local checkbox = frame or create({
						...
						},
						{
							...
							"ImageLabel",
							{
								...
								BackgroundTransparency = 1,
								BorderSizePixel = 0,
								Image = Main.ResolveAsset("rbxassetid://6234266378"),
								Parent = {
									9
								},
								Position = UDim2.new(0.5, 0, 0.5, 0),
						...
						},
						{
							...
							"ImageLabel",
							{
								...
								BackgroundColor3 = Color3.new(1, 1, 1),
								BackgroundTransparency = 1,
								Image = Main.ResolveAsset("rbxassetid://6401617475"),
						...
						},
						{
							...
							"ImageLabel",
							{
								BackgroundColor3 = Color3.new(1, 1, 1),
								BackgroundTransparency = 1,
								Image = Main.ResolveAsset("rbxassetid://6425281788"),
								ImageTransparency = 0.20000000298023,
								Name = "middle",
								Parent = {
									4
					...
					local gui = create({
						...
						},
						{
							...
							"ImageLabel",
							{
								...
								BackgroundTransparency = 1,
								BorderSizePixel = 0,
								Image = Main.ResolveAsset("rbxassetid://1281023007"),
					...
					local guiContents = create({
						...
						},
						{
							...
							"ImageLabel",
							{
								...
								BorderColor3 = Color3.new(0.37647062540054, 0.37647062540054, 0.37647062540054),
								BorderSizePixel = 0,
								Image = Main.ResolveAsset("rbxassetid://1072518406"),
								Name = "ColorSpace",
								Parent = {
									18
								},
						...
						},
						{
							...
							"ImageLabel",
							{
								BackgroundColor3 = Color3.new(1, 1, 1),
								BorderColor3 = Color3.new(0.21568627655506, 0.21568627655506, 0.21568627655506),
								Image = Main.ResolveAsset("rbxassetid://1072518502"),
								Name = "ColorStrip",
								Parent = {
									1
								},
			...
			Lib.ClickSystem = (function()
				...
				funcs.Trigger = function(self, item, button, X, Y)
					if table.find(self.AllowedButtons, button) then
								...
								if self.InputDown then
									...
									while self.InputDown and (not Explorer.Dragging) do
										if tick() - self.InputDown >= 0.4 then
											self.InputDown = false;
											local holdX, holdY = X, Y;
											if Main and Main.Mouse then
												holdX, holdY = Main.Mouse.X, Main.Mouse.Y;
											end;
											self.OnRelease:Fire(item, self.Combo, 2, Vector2.new(holdX, holdY));
											break;
										end;
										task.wait();
									end;
						...
						local release;
						release = Main.TrackConn(service.UserInputService.InputEnded:Connect(function(input)
...
Main = (function()
	...
	Main.AppControls = {};
	Main.Apps = Apps;
	Main.SettingsFile = "DexSettings.json";
	Main.MenuApps = {};
	Main.ModuleErrors = {};
	Main.StubbedApps = {};
	Main.ConnList = {};
	Main.GuiList = {};
	Main.IconOverrides = {
		["http://www.roblox.com/asset/?id=6511490623"] = "NewestIcons.png",
		["rbxassetid://6511490623"] = "NewestIcons.png",
		["rbxassetid://6579106223"] = "icons.png",
		...
		["DexPlusIconDraft"] = "icondraft.png",
	};
	...
	Main.TrackConn = function(conn)
		if conn then
			Main.ConnList[(#Main.ConnList) + 1] = conn;
		end;
		return conn;
	end;
	Main.TrackGui = function(gui)
		...
		for _, trackedGui in ipairs(Main.GuiList) do
			if trackedGui == gui then
				return gui;
			end;
		end;
		Main.GuiList[(#Main.GuiList) + 1] = gui;
		return gui;
	end;
	Main.ResolveAsset = function(assetId)
		...
		if override then
			...
			local path = "dex/assets/" .. override;
			local cached = Main.ResolvedAssetCache and Main.ResolvedAssetCache[path];
			if cached then
				return cached;
			end;
			...
			local function ensureFile()
				...
				local ok, bytes = pcall(game.HttpGet, game, Main.IconSourceBaseUrl .. override);
	...
	Main.EnsureAllIcons = function(forceRefresh)
		...
		if force then
			if table.clear then
				table.clear(Main.ResolvedAssetCache);
			else
				Main.ResolvedAssetCache = {};
			end;
		else
		...
		for _, fileName in pairs(Main.IconOverrides) do
			...
			local path = "dex/assets/" .. fileName;
			...
			if not needDownload and env.isfile then
				...
				local fileExists = ok and exists;
				if fileExists then
					Main.IconFileReady[path] = true;
				end;
			...
			if needDownload then
				local ok, data = pcall(game.HttpGet, game, Main.IconSourceBaseUrl .. fileName);
				if ok and type(data) == "string" then
					local okWrite = pcall(env.writefile, path, data);
					if okWrite then
						Main.IconFileReady[path] = true;
					end;
				end;
	...
	Main.Error = function(str)
		...
		elseif printconsole then
			printconsole("DEX ERROR: " .. tostring(str));
		else
			error("DEX ERROR: " .. tostring(str));
		end;
	end;
	Main.Warn = function(str)
		local message = "DEX WARN: " .. tostring(str);
		if rconsoleprint then
	...
	local function createAppStub(name)
		local noop = function(...)
		end;
		local windowStub = {
			Show = noop,
	...
	local function isStubModule(mod)
		return type(mod) == "table" and mod.__is_stub == true;
	end;
		...
		if EmbeddedModules and EmbeddedModules[name] then
			...
			if script then
				...
				if modulesFolder then
					module = modulesFolder:FindFirstChild(name) or modulesFolder:WaitForChild(name, 2);
				end;
			end;
			if not module then
	...
	Main.LoadModules = function()
		...
		for _, name in ipairs(Main.ModuleList) do
			local success, err = pcall(Main.LoadModule, name);
			if not success then
				local message = ("FAILED LOADING %s CAUSE %s"):format(name, err);
				Main.ModuleErrors[name] = message;
				Main.StubbedApps[name] = true;
				Main.Warn(message);
				Apps[name] = Apps[name] or createAppStub(name);
			end;
		end;
		Explorer = Apps.Explorer or createAppStub("Explorer");
		Properties = Apps.Properties or createAppStub("Properties");
		ScriptViewer = Apps.ScriptViewer or createAppStub("ScriptViewer");
		Notebook = Apps.Notebook or createAppStub("Notebook");
		local appTable = {
			Explorer = Explorer,
	...
	Main.LoadSettings = function()
		local function toColor3(value)
			...
			local function toNumber(component)
			...
			if type(value) == "table" then
				...
				local hex = value.Hex or value.hex;
				if type(hex) == "string" then
					return toColor3(hex);
				end;
				local nested = value.Color or value.color or value.Value or value.value;
				if nested ~= nil then
					local nestedColor = toColor3(nested);
					if nestedColor then
						return nestedColor;
					end;
				end;
			elseif type(value) == "string" then
				...
				elseif hex:match("^%x%x%x$") then
					local rHex = hex:sub(1, 1);
					local gHex = hex:sub(2, 2);
					local bHex = hex:sub(3, 3);
					return Color3.fromRGB(tonumber(rHex .. rHex, 16), tonumber(gHex .. gHex, 16), tonumber(bHex .. bHex, 16));
				end;
			end;
		end;
		...
		local function coerceByTemplate(value, templateValue, fallback)
			if templateValue ~= nil then
				if typeof(templateValue) == "Color3" then
					return toColor3(value) or fallback or templateValue;
				elseif typeof(templateValue) == "Vector3" then
					return toVector3(value) or fallback or templateValue;
				elseif type(templateValue) == "table" then
					if type(value) == "table" then
						local fallbackTable = type(fallback) == "table" and fallback or nil;
						for subKey, subTemplate in pairs(templateValue) do
							if subKey ~= "_Recurse" and value[subKey] ~= nil then
								local subFallback = fallbackTable and fallbackTable[subKey] or nil;
								value[subKey] = coerceByTemplate(value[subKey], subTemplate, subFallback);
							end;
						end;
						return value;
					end;
					return fallback or templateValue;
				end;
			end;
			if value == nil then
				return fallback;
		...
		local function apply(src, dest, template)
			for key, val in pairs(src) do
				local destVal = dest[key];
				local templateVal = template and template[key] or destVal;
				if type(val) == "table" and type(destVal) == "table" and destVal._Recurse then
					templateVal = templateVal or destVal;
					apply(val, destVal, templateVal);
				else
					dest[key] = coerceByTemplate(val, templateVal, destVal);
				end;
			end;
		end;
		Main.ResetSettings();
		...
		if s and data and data ~= "" then
			local success, decoded = pcall(service.HttpService.JSONDecode, service.HttpService, data);
			if success and type(decoded) == "table" then
				apply(decoded, Settings, DefaultSettings);
			else
				Main.Warn("Failed to decode " .. Main.SettingsFile .. ": " .. tostring(decoded));
			end;
		end;
		...
		local defaultWindow = DefaultSettings.Window or {};
		local transparency = tonumber(windowSettings.Transparency);
		if transparency == nil then
			transparency = defaultWindow.Transparency or 0;
		end;
		windowSettings.Transparency = math.clamp(transparency, 0, 1);
		if type(Settings.Decompiler) ~= "table" then
			Settings.Decompiler = {};
		end;
		if type(Settings.Decompiler.UseLuaExpert) ~= "boolean" then
			Settings.Decompiler.UseLuaExpert = DefaultSettings.Decompiler.UseLuaExpert;
		end;
	end;
		...
		local function serialize(val)
			...
			elseif valType == "table" then
				...
				for key, value in pairs(val) do
					newTbl[key] = serialize(value);
				end;
				return newTbl;
			end;
			return val;
		end;
		local ok, encoded = pcall(service.HttpService.JSONEncode, service.HttpService, serialize(Settings));
		if ok then
			return encoded;
		end;
		Main.Warn("Failed to encode Dex settings: " .. tostring(encoded));
	end;
	Main.SaveCurrentSettings = function()
		if not (env and env.writefile) then
			return false;
		end;
		local encoded = Main.ExportSettings();
		if not encoded then
			return false;
		end;
		local ok, err = pcall(env.writefile, Main.SettingsFile, encoded);
		if not ok then
			Main.Warn("Failed writing " .. Main.SettingsFile .. ": " .. tostring(err));
		end;
		return ok;
	end;
	Main.FetchAPI = function()
		...
		if Main.Elevated then
			...
			if not rawAPI then
				...
				local requestData;
				task.spawn(function()
					local ok, response = pcall(game.HttpGet, game, "http://setup.roblox.com/" .. Main.RobloxVersion .. "-API-Dump.json");
		...
		local gui = create({
			...
			},
			{
				...
				"ImageLabel",
				{
					BackgroundColor3 = Color3.new(1, 1, 1),
					BackgroundTransparency = 1,
					Image = Main.ResolveAsset("rbxassetid://2764171053"),
	...
	Main.CreateApp = function(data)
		...
		elseif type(iconIndex) == "string" then
			...
			app.Main.Icon.Size = data.IconSize or UDim2.new(0, 16, 0, 16);
			app.Main.Icon.Position = data.IconPosition or UDim2.new(0.5, -8, 0, 12);
			if data.ImageRectOffset then
				app.Main.Icon.ImageRectOffset = data.ImageRectOffset;
			end;
			if data.ImageRectSize then
				app.Main.Icon.ImageRectSize = data.ImageRectSize;
			end;
		else
			app.Main.Icon.Image = "";
			app.Main.Icon.ImageRectOffset = Vector2.new(0, 0);
		...
		Main.SetMainGuiOpen = function(val)
			...
			if (not val) and Main.SettingsTransparencySlider then
				Main.SettingsTransparencySlider.Visible = false;
			end;
			if val then
		...
		if not val then
			if Main.SettingsMenuContext and Main.SettingsMenuContext.Gui and Main.SettingsMenuContext.Gui.Parent then
				Main.SettingsMenuContext:Hide();
			end;
			if Main.InfoMenuContext and Main.InfoMenuContext.Gui and Main.InfoMenuContext.Gui.Parent then
				Main.InfoMenuContext:Hide();
			end;
			if Main.SettingsTransparencySlider then
				Main.SettingsTransparencySlider.Visible = false;
			end;
			local startTime = tick();
			...
			Main.MainGuiMouseEvent = Main.TrackConn(service.UserInputService.InputBegan:Connect(function(input)
				local clickedSettingsMenu = Main.SettingsMenuContext and Main.SettingsMenuContext.GuiElems and Main.SettingsMenuContext.GuiElems.Main and Lib.CheckMouseInGui(Main.SettingsMenuContext.GuiElems.Main);
				local clickedInfoMenu = Main.InfoMenuContext and Main.InfoMenuContext.GuiElems and Main.InfoMenuContext.GuiElems.Main and Lib.CheckMouseInGui(Main.InfoMenuContext.GuiElems.Main);
				local clickedTransparencySlider = Main.SettingsTransparencySlider and Main.SettingsTransparencySlider.Visible and Lib.CheckMouseInGui(Main.SettingsTransparencySlider);
				if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and (not Lib.CheckMouseInGui(Main.MainGui.OpenButton)) and (not Lib.CheckMouseInGui(Main.MainGui.OpenButton.MainFrame)) and (not clickedSettingsMenu) and (not clickedInfoMenu) and (not clickedTransparencySlider) then
					if Main.SettingsTransparencySlider then
						Main.SettingsTransparencySlider.Visible = false;
					end;
	...
	Main.Exit = function(keepDexState)
		...
		if Main.SettingsMenuContext and Main.SettingsMenuContext.Hide then
			pcall(function()
				Main.SettingsMenuContext:Hide();
			end);
			if Main.SettingsMenuContext.CloseEvent then
				pcall(function()
					Main.SettingsMenuContext.CloseEvent:Disconnect();
				end);
				Main.SettingsMenuContext.CloseEvent = nil;
			end;
			if Main.SettingsMenuContext.Gui then
				pcall(function()
					Main.SettingsMenuContext.Gui:Destroy();
				end);
			end;
		end;
		if Main.InfoMenuContext and Main.InfoMenuContext.Hide then
			pcall(function()
				Main.InfoMenuContext:Hide();
			end);
			if Main.InfoMenuContext.CloseEvent then
				pcall(function()
					Main.InfoMenuContext.CloseEvent:Disconnect();
				end);
				Main.InfoMenuContext.CloseEvent = nil;
			end;
			if Main.InfoMenuContext.Gui then
				pcall(function()
					Main.InfoMenuContext.Gui:Destroy();
				end);
			end;
		end;

		if Main.ConnList then
			for _, conn in ipairs(Main.ConnList) do
				pcall(function()
					if conn then
						conn:Disconnect();
					end;
		...
		if Main.GuiList then
			for _, gui in ipairs(Main.GuiList) do
				pcall(function()
		...
		Main.SettingsMenuContext = nil;
		Main.InfoMenuContext = nil;
		Main.SettingsTransparencySlider = nil;
		Main.Apps = {};
		Main.AppControls = {};
		Main.MenuApps = {};
		Main.ModuleErrors = {};
		...
		if Main.__DexState then
			if keepDexState == true then
				...
				if Main.SessionId then
					Main.__DexState.SessionId = Main.SessionId;
				end;
				DexState.Sync(Main.__DexState);
			else
				Main.__DexState.State = "dead";
				DexState.Clear(Main.__DexState);
				Main.__DexState = nil;
			end;
		end;
	end;
	Main.CreateMainGui = function()
		local gui = create({
			...
			},
			{
				...
				"ImageLabel",
				{
					BackgroundColor3 = Color3.new(1, 1, 1),
					BackgroundTransparency = 1,
					Image = Main.ResolveAsset("rbxassetid://6578871732"),
					ImageTransparency = 0.20000000298023,
					Name = "Icon",
					Parent = {
						10
			...
			},
			{
				...
				"ImageLabel",
				{
					BackgroundColor3 = Color3.new(1, 1, 1),
					BackgroundTransparency = 1,
					Image = Main.ResolveAsset("rbxassetid://6578933307"),
					ImageTransparency = 0.20000000298023,
					Name = "Icon",
					Parent = {
						12
			...
			},
			{
				...
				"ImageLabel",
				{
					BackgroundColor3 = Color3.new(1, 1, 1),
					BackgroundTransparency = 1,
					Image = Main.ResolveAsset("rbxassetid://6579106223"),
		...
		local settingsButton = openButton.MainFrame.BottomFrame.Settings;
		local infoButton = openButton.MainFrame.BottomFrame.Information;
		...
		local settingsMenu = Lib.ContextMenu.new();
		settingsMenu.Iconless = true;
		settingsMenu.Width = 210;
		Main.SettingsMenuContext = settingsMenu;
		local infoMenu = Lib.ContextMenu.new();
		infoMenu.Iconless = true;
		infoMenu.Width = 260;
		Main.InfoMenuContext = infoMenu;
		local function applySettingChanges()
			if Explorer and Explorer.SetSortingEnabled then
				pcall(Explorer.SetSortingEnabled, Settings.Explorer.Sorting);
			end;
			if Explorer and Explorer.SetAutoUpdateSearch then
				pcall(Explorer.SetAutoUpdateSearch, Settings.Explorer.AutoUpdateSearch);
			end;
			if Explorer and Explorer.ForceUpdate then
				pcall(Explorer.ForceUpdate);
			end;
			if Explorer and Explorer.Refresh then
		...
		local function getWindowTransparency()
			return math.clamp(tonumber(Settings.Window and Settings.Window.Transparency) or 0, 0, 1);
		end;
		local transparencySlider = createSimple("Frame", {
			AnchorPoint = Vector2.new(1, 0),
		...
		local transparencyLabel = createSimple("TextLabel", {
			BackgroundTransparency = 1,
			Font = Enum.Font.SourceSans,
			Name = "Title",
			Parent = transparencySlider,
		...
		local transparencyValue = createSimple("TextLabel", {
			BackgroundTransparency = 1,
			Font = Enum.Font.SourceSans,
			Name = "Value",
			Parent = transparencySlider,
		...
		local transparencyTrack = createSimple("TextButton", {
			AutoButtonColor = false,
		...
		local transparencyFill = createSimple("Frame", {
		...
		local transparencyKnob = createSimple("Frame", {
			AnchorPoint = Vector2.new(0.5, 0.5),
		...
		createSimple("UICorner", {
			...
			Parent = transparencyKnob
		});
		local transparencyHint = createSimple("TextLabel", {
			BackgroundTransparency = 1,
			Font = Enum.Font.SourceSans,
			Name = "Hint",
			Parent = transparencySlider,
			...
			Text = "Drag to adjust window background transparency",
			TextColor3 = Settings.Theme.PlaceholderText,
			TextSize = 12,
			TextXAlignment = Enum.TextXAlignment.Left,
			ZIndex = 1000002
		});
		Main.SettingsTransparencySlider = transparencySlider;
		local transparencyDragging = false;
		local function updateTransparencySliderVisual()
			local transparency = getWindowTransparency();
			transparencyFill.Size = UDim2.new(transparency, 0, 1, 0);
			transparencyKnob.Position = UDim2.new(transparency, 0, 0.5, 0);
			transparencyValue.Text = ("%d%%"):format(math.floor(transparency * 100 + 0.5));
			transparencySlider.BackgroundColor3 = Settings.Theme.Main2;
			transparencySlider.BorderColor3 = Settings.Theme.Outline1;
			transparencyLabel.TextColor3 = Settings.Theme.Text;
			transparencyValue.TextColor3 = Settings.Theme.PlaceholderText;
			transparencyTrack.BackgroundColor3 = Settings.Theme.TextBox;
			transparencyTrack.BorderColor3 = Settings.Theme.Outline3;
			transparencyFill.BackgroundColor3 = Settings.Theme.ListSelection;
			transparencyKnob.BackgroundColor3 = Settings.Theme.Text;
			transparencyHint.TextColor3 = Settings.Theme.PlaceholderText;
		end;
		local function setTransparencyValue(value)
			value = math.clamp(math.floor(value * 100 + 0.5) / 100, 0, 1);
			Settings.Window.Transparency = value;
			updateTransparencySliderVisual();
			if Lib and Lib.RefreshTheme then
				pcall(Lib.RefreshTheme);
			end;
			persistCurrentSettings();
			rebuildSettingsMenu();
			if settingsMenu.Gui.Parent then
				settingsMenu:Refresh();
			end;
		end;
		local function setTransparencyFromPosition(positionX)
			local absolutePosition = transparencyTrack.AbsolutePosition.X;
			local absoluteSize = math.max(1, transparencyTrack.AbsoluteSize.X);
			setTransparencyValue((positionX - absolutePosition) / absoluteSize);
		end;
		local function showTransparencySlider()
			updateTransparencySliderVisual();
			local menuMain = settingsMenu.GuiElems and settingsMenu.GuiElems.Main;
			if settingsMenu.Gui.Parent and menuMain then
				local menuPos = menuMain.AbsolutePosition;
				local menuSize = menuMain.AbsoluteSize;
				transparencySlider.Position = UDim2.new(0, menuPos.X + menuSize.X, 0, menuPos.Y + menuSize.Y + 6);
			else
				local btnPos = settingsButton.AbsolutePosition;
				local btnSize = settingsButton.AbsoluteSize;
				transparencySlider.Position = UDim2.new(0, btnPos.X + btnSize.X, 0, btnPos.Y + btnSize.Y + 6);
			end;
			transparencySlider.Visible = true;
		end;
		local function hideTransparencySlider()
			transparencyDragging = false;
			transparencySlider.Visible = false;
		end;
		local function rebuildInfoMenu()
			infoMenu:Clear();
			infoMenu:AddDivider("Credits");
			infoMenu:Add({
				Name = "Dex created by Moon"
			});
			infoMenu:Add({
				Name = "Fixed, updated, and reworked by ltseverydayyou"
			});
		end;
		transparencyTrack.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				transparencyDragging = true;
				setTransparencyFromPosition(input.Position.X);
			end;
		end);
		transparencyTrack.InputChanged:Connect(function(input)
		...
		Main.TrackConn(service.UserInputService.InputEnded:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				transparencyDragging = false;
			end;
		end));
		local function makeToggleLabel(name, value)
			return (value and "[ON] " or "[OFF] ") .. name;
		end;
		rebuildSettingsMenu = function()
			settingsMenu:Clear();
			settingsMenu:AddDivider("Settings");
			settingsMenu:Add({
				Name = makeToggleLabel("Explorer Sorting", Settings.Explorer.Sorting),
				NoHide = true,
				OnClick = function()
					Settings.Explorer.Sorting = not Settings.Explorer.Sorting;
					applySettingChanges();
					persistCurrentSettings();
					rebuildSettingsMenu();
					settingsMenu:Refresh();
			...
			settingsMenu:Add({
				Name = makeToggleLabel("Use Name Width", Settings.Explorer.UseNameWidth),
				NoHide = true,
				OnClick = function()
					Settings.Explorer.UseNameWidth = not Settings.Explorer.UseNameWidth;
					applySettingChanges();
					persistCurrentSettings();
					rebuildSettingsMenu();
					settingsMenu:Refresh();
			...
			settingsMenu:Add({
				Name = makeToggleLabel("Search Auto Update", Settings.Explorer.AutoUpdateSearch),
				NoHide = true,
				OnClick = function()
					Settings.Explorer.AutoUpdateSearch = not Settings.Explorer.AutoUpdateSearch;
					applySettingChanges();
					persistCurrentSettings();
					rebuildSettingsMenu();
					settingsMenu:Refresh();
			...
			settingsMenu:Add({
				Name = makeToggleLabel("Keyboard Shortcuts", Settings.Explorer.KeyboardShortcuts ~= false),
				NoHide = true,
				OnClick = function()
					Settings.Explorer.KeyboardShortcuts = Settings.Explorer.KeyboardShortcuts == false;
					persistCurrentSettings();
					rebuildSettingsMenu();
					settingsMenu:Refresh();
				end
			...
			settingsMenu:Add({
				Name = ("UI Transparency: %d%%"):format(math.floor(getWindowTransparency() * 100 + 0.5)),
				OnClick = function()
					showTransparencySlider();
				end
			});
			settingsMenu:Add({
				Name = makeToggleLabel("Use lua.expert Decompiler", Settings.Decompiler.UseLuaExpert),
				NoHide = true,
				OnClick = function()
					Settings.Decompiler.UseLuaExpert = not Settings.Decompiler.UseLuaExpert;
					persistCurrentSettings();
					rebuildSettingsMenu();
					settingsMenu:Refresh();
				end
			...
			settingsMenu:Add({
				Name = makeToggleLabel("Properties Hidden", Settings.Properties.ShowHidden),
				NoHide = true,
				OnClick = function()
					Settings.Properties.ShowHidden = not Settings.Properties.ShowHidden;
					applySettingChanges();
					persistCurrentSettings();
					rebuildSettingsMenu();
					settingsMenu:Refresh();
			...
			settingsMenu:Add({
				Name = makeToggleLabel("Properties Deprecated", Settings.Properties.ShowDeprecated),
				NoHide = true,
				OnClick = function()
					Settings.Properties.ShowDeprecated = not Settings.Properties.ShowDeprecated;
					applySettingChanges();
					persistCurrentSettings();
					rebuildSettingsMenu();
					settingsMenu:Refresh();
		...
		local function showSettingsMenu()
			...
			if infoMenu.Gui.Parent then
				infoMenu:Hide();
			end;
			rebuildSettingsMenu();
			local btnPos = settingsButton.AbsolutePosition;
			local btnSize = settingsButton.AbsoluteSize;
			settingsMenu:Show(btnPos.X - 180 + btnSize.X, btnPos.Y + btnSize.Y + 2);
		end;
		local function showInfoMenu()
			...
			rebuildInfoMenu();
			local btnPos = infoButton.AbsolutePosition;
			local btnSize = infoButton.AbsoluteSize;
			infoMenu:Show(btnPos.X - 236 + btnSize.X, btnPos.Y + btnSize.Y + 2);
		end;
		settingsButton.MouseButton1Click:Connect(showSettingsMenu);
		infoButton.MouseButton1Click:Connect(showInfoMenu);
		if Explorer and Explorer.Window and (not isStubModule(Explorer)) then
			Main.CreateApp({
				Name = "Explorer",
		...
		if Properties and Properties.Window and (not isStubModule(Properties)) then
			Main.CreateApp({
				Name = "Properties",
				IconMap = Main.LargeIcons,
				Icon = "Properties",
		...
		if ScriptViewer and ScriptViewer.Window and (not isStubModule(ScriptViewer)) then
			Main.CreateApp({
				Name = "Script Viewer",
				IconMap = Main.LargeIcons,
				Icon = "Script_Viewer",
		...
		if Explorer and (not isStubModule(Explorer)) then
			Main.CreateApp({
				...
				OnClick = function(callback)
					...
					elseif cptsOnMouseClick ~= nil then
		...
		Main.CreateApp({
			Name = "Exit Dex",
			Icon = Main.ResolveAsset("DexPlusIconDraft"),
			ImageRectOffset = Vector2.new(166, 221),
			ImageRectSize = Vector2.new(14, 13),
			IconSize = UDim2.new(0, 32, 0, 30),
			IconPosition = UDim2.new(0.5, -16, 0, 5),
			OnClick = function(isOpen)
				if isOpen then
					Main.Exit();
				end;
			end
	...
	Main.Init = function()
		...
		if env.writefile and env.readfile then
			local hasSettingsFile, existingSettings = pcall(env.readfile, Main.SettingsFile);
			if (not hasSettingsFile) or existingSettings == "" then
				pcall(Main.SaveCurrentSettings);
			end;
		end;
		...
		Main.MiscIcons = Lib.IconMap.new(Main.ResolveAsset("http://www.roblox.com/asset/?id=6511490623"), 256, 256, 16, 16);
		...
		Main.LargeIcons = Lib.IconMap.new(Main.ResolveAsset("rbxassetid://6579106223"), 256, 256, 32, 32);
		...
		if Explorer and Explorer.Init then
			Explorer.Init();
		end;
		if Properties and Properties.Init then
			Properties.Init();
		end;
		if ScriptViewer and ScriptViewer.Init then
			ScriptViewer.Init();
		end;
		Lib.FastWait();
		...
		Lib.Window.Init();
		Main.CreateMainGui();
		local explorerReady = Explorer and Explorer.Window and Explorer.Window.Show and (not isStubModule(Explorer));
		local propertiesReady = Properties and Properties.Window and Properties.Window.Show and (not isStubModule(Properties));
		if explorerReady then
		...
		if explorerReady and propertiesReady then
...
local dexState = DexState.Get(true);
Main.__DexState = dexState;
Main.__DexLoaded = false;
Main.__DexInitStarted = true;
Main.SessionId = Main.SessionId or tostring(math.random(1, 1000000000));

dexState.Main = Main;
dexState.State = "starting";
dexState.StartedAt = os.clock();
dexState.SessionId = Main.SessionId;
DexState.Sync(dexState);
