	...
	Explorer.ShowRightClick = function(MousePos)
		...
		if presentClasses["LuaSourceContainer"] then
			...
			if env.isdecompile ~= nil then
				...
				canDecompile = env.decompile ~= nil or env.getscriptbytecode ~= nil
			end
		...
		context:Register("SAVE_SCRIPT",{Name = "Save Script", IconMap = Explorer.MiscIcons, Icon = "Save", DisabledIcon = "Empty", OnClick = function()
			for _, v in next, selection.List do
				if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
					local success, source, decompileErr = pcall(env.decompile, v.Obj)
					if not success then
						source = ("-- DEX - %s failed to decompile %s\n-- Reason: %s"):format(env.executor, v.Obj.ClassName, tostring(source))
					elseif not source then
						source = ("-- DEX - %s failed to decompile %s\n-- Reason: %s"):format(env.executor, v.Obj.ClassName, tostring(decompileErr or "Unknown Error"))
					end
...
return search]==]
	...
	Explorer.Init = function()
		...
		local explorerItems = create({
			...
			{8,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image=Main.ResolveAsset("rbxassetid://5034718129"),ImageColor3=Color3.new(0.39215686917305,0.39215686917305,0.39215686917305),Parent={7},Size=UDim2.new(0,16,0,16),}},
			...
			{10,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image=Main.ResolveAsset("rbxassetid://5642310344"),Parent={9},Position=UDim2.new(0,3,0,3),Size=UDim2.new(0,12,0,12),}},
