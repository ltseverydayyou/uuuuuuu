...
local function main()
    ...
    local rowCons, rowChk, rowIco, winCons = {}, {}, {}, {}
    ...
    local function addWinCon(c)
        winCons[#winCons + 1] = c
        return c
    end

    local function addPropCon(c)
        propCons[#propCons + 1] = c
        return c
    end
    ...
    Properties.StringToValue = function(prop, str)
        ...
        elseif typeName == "CFrame" then
            ...
            if #v >= 12 then
                ...
                local ok, res = pcall(CFrame.new, unpack(n))
                if ok then
                    return res
                end
            end
        elseif typeName == "Rect" then
            ...
            if #v >= 4 then
                ...
                local ok, res = pcall(Rect.new, unpack(n))
                if ok then
                    return res
                end
            end
        elseif typeName == "Ray" then
            ...
            if #v >= 6 then
                ...
                local ok1, org = pcall(Vector3.new, n[1], n[2], n[3])
                local ok2, dir = pcall(Vector3.new, n[4], n[5], n[6])
                if ok1 and ok2 then
                    return Ray.new(org, dir)
                end
            end
        ...
        elseif typeName == "Color3" then
            ...
            if #v >= 3 then
                ...
                if r and g and b then
                    local ok, res = pcall(Color3.fromRGB, r, g, b)
                    if ok then
                        return res
                    end
                end
    ...
    Properties.UpdateView = function()
        local pf = propsFrame
        if not pf or not pf.Parent then
            return
        end

        local maxY = pf.Parent.AbsoluteSize.Y
        local maxX = pf.Parent.AbsoluteSize.X
        ...
        local maxEnt = math.max(1, math.ceil(maxY / 23))

        local totW = (Properties.ViewWidth + Properties.MinInputWidth)
        scrollV.VisibleSpace = maxEnt
        scrollV.TotalSpace = #viewList + 1

        scrollH.VisibleSpace = maxX
        scrollH.TotalSpace = totW

        scrollV.Gui.Visible = (#viewList + 1) > maxEnt
        scrollH.Gui.Visible = (Settings.Properties.ScaleType == 0) and (totW > maxX)

        local wOff = scrollV.Gui.Visible and -16 or 0
        local hOff = scrollH.Gui.Visible and -39 or -23

        local newSz = UDim2.new(1, wOff, 1, hOff)

        if pf.Size ~= newSz then
            pf.Size = newSz
            task.defer(Properties.UpdateView)
            return
        end
    ...
    Properties.Update = function()
        ...
        local function recur(list, depth)
            for i = 1, #list do
                local prop = list[i]
    ...
    Properties.NewPropEntry = function(index)
        ...
        local function addCon(x)
            rowCons[index][#rowCons[index] + 1] = x
            return x
        end
        ...
        addCon(
            nameFrame.Expand.InputBegan:Connect(
                function(input)
                    ...
                    local fullName =
                        (prop.CategoryName and "CAT_" .. prop.CategoryName) or
            ...
            nameFrame.Expand.MouseButton1Down:Connect(
                function()
                    ...
                    local fullName =
                        ...
                        prop.Class .. "." .. prop.Name .. (prop.SubName or "")
                    if
                        not prop.CategoryName and not Properties.ExpandableTypes[prop.ValueType and prop.ValueType.Name] and
                            not Properties.ExpandableProps[fullName]
                     then
                        return
                    end
            ...
            nameFrame.PropName.InputBegan:Connect(
                function(input)
                    ...
                    if input.UserInputType == Enum.UserInputType.MouseMovement and not nameFrame.PropName.TextFits then
                        ...
                        local sizeX =
                            service.TextService:GetTextSize(
                            dispName,
                            14,
        ...
        addCon(
            valueFrame.RightButton.MouseButton1Click:Connect(
                function()
                    ...
                    local fullName = prop.Class .. "." .. prop.Name .. (prop.SubName or "")
                    local inputFullName =
                        inputProp and (inputProp.Class .. "." .. inputProp.Name .. (inputProp.SubName or ""))
    ...
    Properties.SetSoundPreview = function(soundObj, noplay)
        ...
        if not soundObj then
            ...
            coroutine.wrap(
                function()
                    local pt = tick()
                    Properties.SoundPreviewTime = pt
                    while Properties.SoundPreviewTime == pt do
                        local snd = Properties.PreviewSound
                        if not snd or not snd.Parent then
                            break
                        end
                        local okPlay, playing =
                            pcall(
                            function()
                                return snd.Playing
                            end
                        )
                        if not okPlay or not playing then
                            break
                        end
                        local entry = Properties.GetSoundPreviewEntry()
                        if entry then
                            local okTL, tl =
                                pcall(
                                function()
                                    return snd.TimeLength
                                end
                            )
                            local okTP, tp =
                                pcall(
                                function()
                                    return snd.TimePosition
                                end
                            )
                            if okTL and okTP then
                                local perc = tp / ((tl == 0 and 1) or tl)
    ...
    Properties.IsTextEditable = function(prop)
        local typeData = prop.ValueType
        local typeName = typeData.Name
        return typeName ~= "bool" and typeData.Category ~= "Enum" and typeData.Category ~= "Class" and
            typeName ~= "BrickColor"
    end
    ...
    Properties.DisplayEnumDropdown = function(entryIndex)
        ...
        local sorted = {}
        for _, enumIt in next, enum:GetEnumItems() do
            sorted[#sorted + 1] = enumIt
        end
        table.sort(
            sorted,
            function(a, b)
        ...
        if not editor then
            ...
            editor.OnMoreColors:Connect(
                function()
                    editor:Close()
                    if
                        editor.CurrentProp and editor.CurrentProp.Class == "BasePart" and
                            editor.CurrentProp.Name == "BrickColor"
                     then
    ...
    Properties.DisplayProp = function(prop, entryIndex)
        ...
        if (typeName == "Color3" or typeName == "BrickColor" or typeName == "ColorSequence") then
            ...
            if propVal then
                gradient.Color =
                    (typeName == "Color3" and ColorSequence.new(propVal)) or
                    (typeName == "BrickColor" and ColorSequence.new(propVal.Color)) or
                    propVal
            else
    ...
    local function clrRow(i)
        ...
        if ico then
            pcall(
                function()
                    if Main and Main.MiscIcons and Main.MiscIcons.ReturnLabel then
                        Main.MiscIcons:ReturnLabel(ico)
                    else
                        ico:Destroy()
                    end
                end
    ...
    local function _doRefresh()
        local pf = propsFrame
        local pz = (pf and pf.AbsoluteSize) or Vector2.new(0, 0)
        local maxEntries = math.max(1, math.ceil((pz.Y > 0 and pz.Y or 1) / 23))
        local maxX = (pz.X > 0 and pz.X or 1)
        ...
        if Properties.FullNameFrame then
            Properties.FullNameFrame.Visible = false
        end
        if Properties.FullNameFrameAttach then
            Properties.FullNameFrameAttach.Disable()
        end
            ...
            if prop then
                ...
                if prop.SpecialRow then
                    ...
                    if prop.CategoryName then
                        ...
                        expand.Visible =
                            typeData.Category == "DataType" and Properties.ExpandableTypes[typeName] or
                        ...
                        if
                            prop.CategoryName or Properties.ExpandableTypes[prop.ValueType and prop.ValueType.Name] or
                                Properties.ExpandableProps[gName]
                         then
                        ...
                        if rootTypeName then
                            ...
                            elseif rootTypeName == "CFrame" then
                                local rootPos, rootRight, rootUp, rootLook =
                                    ...
                                    root.LookVector
                                local pos =
                                    (subName == ".Position" and setVal) or
                                    Vector3.new(
                                ...
                                local rightV =
                                    (subName == ".RightVector" and setVal) or
                                    Vector3.new(
                                ...
                                local upV =
                                    (subName == ".UpVector" and setVal) or
                                    Vector3.new(
                                ...
                                local lookV =
                                    (subName == ".LookVector" and setVal) or
                                    Vector3.new(
                                        (subName == ".LookVector.X" and setVal) or rootLook.X,
                                        (subName == ".LookVector.Y" and setVal) or rootLook.Y,
                                        (subName == ".LookVector.Z" and setVal) or rootLook.Z
                                    )
                            ...
                            elseif rootTypeName == "PhysicalProperties" then
                                ...
                                local density =
                                    (subName == ".Density" and setVal) or (root and root.Density) or rootProps.Density
                                local friction =
                                    (subName == ".Friction" and setVal) or (root and root.Friction) or
                                    rootProps.Friction
                                local elasticity =
                                    (subName == ".Elasticity" and setVal) or (root and root.Elasticity) or
                                    rootProps.Elasticity
                                local frictionWeight =
                                    (subName == ".FrictionWeight" and setVal) or (root and root.FrictionWeight) or
                            ...
                            elseif rootTypeName == "Ray" then
                                ...
                                local direction =
                                    (subName == ".Direction" and setVal) or
                                    Vector3.new(
    ...
    Properties.Init = function()
        ...
        Properties.GuiElems.ScrollCorner = guiItems.ScrollCorner
        ...
        Properties.ApplyTheme = function()
            ...
            if g.ToolBar then
                g.ToolBar.BackgroundColor3 = t.Main2 or g.ToolBar.BackgroundColor3
            end
            if g.PropsFrame then
                g.PropsFrame.BackgroundColor3 = t.Main1 or g.PropsFrame.BackgroundColor3
            end
            if g.ScrollCorner then
                g.ScrollCorner.BackgroundColor3 = t.Main1 or g.ScrollCorner.BackgroundColor3
            end
        end
    end

    Properties.Destroy = function()
        local pt = Properties.SoundPreviewTime
        Properties.SoundPreviewTime = (pt and (pt + 1)) or tick()
        ...
        if Properties.FullNameFrame then
            pcall(
                function()
                    Properties.FullNameFrame:Destroy()
                end
            )
        end
        Properties.FullNameFrame = nil
        Properties.FullNameFrameAttach = nil
        Properties.FullNameFrameIndex = nil

        if Properties.PreviewSound then
            pcall(
                function()
                    Properties.PreviewSound:Destroy()
                end
            )
            Properties.PreviewSound = nil
        end

        local ed = Properties.EnumDropdown
        Properties.EnumDropdown = nil
        closeObj(ed)

        ed = Properties.BrickColorEditor
        Properties.BrickColorEditor = nil
        closeObj(ed)

        ed = Properties.ColorEditor
        Properties.ColorEditor = nil
        closeObj(ed)

        ed = Properties.NumberSequenceEditor
        Properties.NumberSequenceEditor = nil
        closeObj(ed)

        ed = Properties.ColorSequenceEditor
        Properties.ColorSequenceEditor = nil
        closeObj(ed)

        ed = Properties.EnumContext
        Properties.EnumContext = nil
        closeObj(ed)

        ed = Properties.AttributeContext
