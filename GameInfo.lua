...
local function ClonedService(name)
	...
	local Reference = cloneref or function(r)
		return r;
	end;
...
local function ripple(btn)
	...
	btn.InputBegan:Connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
			...
			(TweenService:Create(c0, TweenInfo.new(0.35, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local function makeTextButton(txt, baseColor)
	...
	st.Thickness = 1;
	st.Transparency = 0.5;
	st.Color = lerpColor(baseColor, Color3.new(0, 0, 0), 0.5);
	st.Parent = b;
	b.MouseEnter:Connect(function()
		(TweenService:Create(b, TweenInfo.new(0.12), {
			BackgroundColor3 = lerpColor(baseColor, Color3.new(1, 1, 1), 0.06)
		})):Play();
	end);
...
local btnRefresh = makeTextButton("🔃", c.ux);
btnRefresh.Parent = ctrl;
local btnMin = makeTextButton("V", c.warn);
btnMin.Parent = ctrl;
local btnClose = makeTextButton("X", c.stop);
btnClose.Parent = ctrl;
...
local function card(h, borderColor)
	...
	st.Color = borderColor or Color3.fromRGB(64, 66, 74);
	st.Parent = f;
	return f;
end;
local function autoRowHeight(row, kLbl, vLbl, leftScale)
	...
	local rw = w - lw - 12;
	local kh = (TextService:GetTextSize(kLbl.Text, kLbl.TextSize, kLbl.Font, Vector2.new(lw, 1000000))).Y;
	local vh = (TextService:GetTextSize(vLbl.Text, vLbl.TextSize, vLbl.Font, Vector2.new(rw, 1000000))).Y;
...
local function addRow(k, v)
	...
	task.defer(function()
		...
		(kLbl:GetPropertyChangedSignal("Text")):Connect(function()
			autoRowHeight(r, kLbl, vLbl, 0.35);
		end);
		(vLbl:GetPropertyChangedSignal("Text")):Connect(function()
			autoRowHeight(r, kLbl, vLbl, 0.35);
		end);
		r.Visible = true;
		r.BackgroundTransparency = 1;
		(TweenService:Create(r, TweenInfo.new(0.18), {
...
local function addDropdown(titleText, tbl)
	...
	local function rebuild()
		...
		task.defer(function()
			...
			(TweenService:Create(contentFrame, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				Size = UDim2.new(1, -24, 0, h)
			})):Play();
			(TweenService:Create(container, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
btnMin.MouseButton1Click:Connect(function()
	...
	if minimized then
		(TweenService:Create(btnMin, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Rotation = 180
		})):Play();
		(TweenService:Create(gIcon, TweenInfo.new(0.12), {
			Size = UDim2.fromOffset(0, 0)
		})):Play();
		(TweenService:Create(gName, TweenInfo.new(0.12), {
			TextTransparency = 1
		})):Play();
		(TweenService:Create(gOwner, TweenInfo.new(0.12), {
			TextTransparency = 1
		})):Play();
		task.wait(0.12);
		header.Visible = false;
		(TweenService:Create(content, TweenInfo.new(0.18), {
			Size = UDim2.new(1, 0, 0, 0)
		})):Play();
		(TweenService:Create(root, TweenInfo.new(0.18), {
			Size = UDim2.new(originalSize.X.Scale, originalSize.X.Offset, 0, 54)
		})):Play();
	else
		header.Visible = true;
		...
		(TweenService:Create(gIcon, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = UDim2.fromOffset(78, 78)
		})):Play();
		(TweenService:Create(gName, TweenInfo.new(0.18), {
			TextTransparency = 0
		})):Play();
		(TweenService:Create(gOwner, TweenInfo.new(0.18), {
			TextTransparency = 0
		})):Play();
		(TweenService:Create(btnMin, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Rotation = 0
...
btnClose.MouseButton1Click:Connect(function()
	(TweenService:Create(root, TweenInfo.new(0.12), {
		BackgroundTransparency = 1
	})):Play();
...
local function displayGameInfo()
	...
	local function mark(k)
		seen[string.lower(k)] = true;
	end;
	local function seenKey(k)
		return seen[string.lower(k)] == true;
	end;
	...
	local cdrop = {};
	if creator then
		for ck, cv in pairs(creator) do
			cdrop[ck] = cv;
		end;
	end;
	if next(cdrop) == nil and gi and typeof(gi.Creator) == "table" then
		for ck, cv in pairs(gi.Creator) do
			cdrop[ck] = cv;
		end;
	end;
...
btnRefresh.MouseButton1Click:Connect(function()
	(TweenService:Create(btnRefresh, TweenInfo.new(0.32, Enum.EasingStyle.Linear), {
		Rotation = btnRefresh.Rotation + 360
	})):Play();
