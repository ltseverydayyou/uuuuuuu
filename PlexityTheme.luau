		...
		if isf(cfgPath) then
			...
			if ok and type(raw) == "string" then
				...
				if ok2 and type(decoded) == "table" then
					...
					if type(decoded.sH) == "number" then
						data.start.h = decoded.sH;
					end;
					if type(decoded.sV) == "number" then
						data.start.v = decoded.sV;
					end;
					if type(decoded.eH) == "number" then
						data.finish.h = decoded.eH;
					end;
					if type(decoded.eV) == "number" then
						data.finish.v = decoded.eV;
					end;
				end;
			end;
		end;
...
local function saveCfg(data)
	if fsok then
		ensureCfgDir();
		pcall(function()
			wf(cfgPath, __lt.cm("HttpService", "JSONEncode", data));
		end);
	end;
end;
local PT = {
...
local function rescan()
	task.spawn(function()
		...
		local desc = target:QueryDescendants("Instance");
		for i = 1, #desc do
			qAdd(desc[i]);
			if i % 200 == 0 then
				Wait();
...
local function onDescAdded(o)
	task.spawn(function()
		qAdd(o);
		local desc = o:QueryDescendants("Instance");
		for i = 1, #desc do
			qAdd(desc[i]);
			if i % 100 == 0 then
				Wait();
...
tb.Position = UDim2.new(0.5, 0, 0.02, 0);
tb.Size = UDim2.new(0.05, 0, 0.06, 0);
...
tb.Text = "v";
(Instance.new("UICorner", tb)).Name = "\000";
(tb:FindFirstChildOfClass("UICorner")).CornerRadius = UDim.new(1, 0);
(Instance.new("UIStroke", tb)).Name = "\000";
(tb:FindFirstChildOfClass("UIStroke")).Color = Color3.fromRGB(70, 70, 70);
local fr = Instance.new("Frame", sg);
fr.Name = "\000";
fr.AnchorPoint = Vector2.new(0.5, 0);
fr.Position = UDim2.new(0.5, 0, 0.1, 0);
fr.Size = UDim2.new(0.35, 0, 0.6, 0);
fr.BackgroundColor3 = Color3.fromRGB(30, 30, 30);
fr.BorderSizePixel = 0;
fr.Active = true;
fr.Visible = false;
(Instance.new("UICorner", fr)).Name = "\000";
(fr:FindFirstChildOfClass("UICorner")).CornerRadius = UDim.new(0.05, 0);
(Instance.new("UIStroke", fr)).Name = "\000";
(fr:FindFirstChildOfClass("UIStroke")).Color = Color3.fromRGB(70, 70, 70);
...
tl.Active = true;
(Instance.new("UICorner", tl)).Name = "\000";
(tl:FindFirstChildOfClass("UICorner")).CornerRadius = UDim.new(0.05, 0);
...
local function mk(label, isV, getV, setV, getH)
	...
	lt.Name = "\000";
	lt.Size = UDim2.new(0.5, 0, 0.05, 0);
	lt.Position = UDim2.new(0.05, 0, y, 0);
	...
	tr.Name = "\000";
	tr.Size = UDim2.new(0.9, 0, 0.05, 0);
	tr.Position = UDim2.new(0.05, 0, y + 0.05, 0);
	tr.Active = true;
	tr.BorderSizePixel = 0;
	tr.BackgroundColor3 = isV and Color3.fromHSV(getH(), 1, getV()) or Color3.new(1, 1, 1);
	(Instance.new("UICorner", tr)).Name = "\000";
	(tr:FindFirstChildOfClass("UICorner")).CornerRadius = UDim.new(0.02, 0);
	...
	hd.Name = "\000";
	hd.Size = UDim2.new(0.04, 0, 1, 0);
	hd.BackgroundColor3 = Color3.fromRGB(240, 240, 240);
	hd.Active = true;
	(Instance.new("UICorner", hd)).Name = "\000";
	(hd:FindFirstChildOfClass("UICorner")).CornerRadius = UDim.new(0.5, 0);
	(Instance.new("UIStroke", hd)).Name = "\000";
	(hd:FindFirstChildOfClass("UIStroke")).Color = Color3.fromRGB(150, 150, 150);
	local entry = {
		track = tr,
		grad = gr,
		handle = hd,
...
mk("Start Hue", false, function()
	return PT.data.start.h;
end, function(v)
	PT.data.start.h = v;
end, function()
	return PT.data.start.h;
end);
mk("Start Brightness", true, function()
	return PT.data.start.v;
end, function(v)
	PT.data.start.v = v;
end, function()
	return PT.data.start.h;
end);
mk("End Hue", false, function()
	return PT.data.finish.h;
end, function(v)
	PT.data.finish.h = v;
end, function()
	return PT.data.finish.h;
end);
mk("End Brightness", true, function()
	return PT.data.finish.v;
end, function(v)
	PT.data.finish.v = v;
end, function()
	return PT.data.finish.h;
