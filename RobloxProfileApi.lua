if not isfolder or (not makefolder) or (not isfile) or (not delfile) or (not listfiles) then
	return;
end;
if not isfolder("ExecutorProfiles") then
	makefolder("ExecutorProfiles");
end;
...
local function getsettingprofiles(setting)
	...
	if issetting(setting) then
		...
		local s, list = pcall(listfiles, path);
		for i = 1, #list do
