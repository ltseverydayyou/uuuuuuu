...
local set_thread_identity = (syn and syn.set_thread_identity)
    or setidentity
...
local rawMakeWritable = makewritable
    or makewriteable
    or make_writeable
local rawMakeReadonly = makereadonly or make_readonly
local canToggleReadonly = type(setreadonly) == 'function'
    or type(rawMakeWritable) == 'function'
local makewritable = rawMakeWritable
    or function(tbl)
        if type(setreadonly) == 'function' then
            setreadonly(tbl, false)
        end
    end
local makereadonly = rawMakeReadonly
    or function(tbl)
        if type(setreadonly) == 'function' then
...
local hookmetamethod = hookmetamethod
    or (
        getrawmetatable
        and (type(hookfunction) == 'function' or canToggleReadonly)
    )
...
local function SafeGetService(name)
    local Service = function(_, serviceName) return __lt.gs(serviceName); end;local Reference = cloneref
...
local function protectUI(sGui)
	local protected = __NAProtectUI(sGui)
	if protected then
		return protected
	end
...
local Highlight = {}
do
    local syntaxMaxChars = math.max(0, tonumber(rawget(getgenv(), 'SimpleSpySyntaxMaxChars')) or 20000)
    local displayMaxChars = math.max(1000, tonumber(rawget(getgenv(), 'SimpleSpyCodeDisplayMaxChars')) or 40000)
    local colors = {
        plain = Color3.fromRGB(248, 248, 248),
        keyword = Color3.fromRGB(255, 120, 160),
        global = Color3.fromRGB(132, 203, 255),
        string = Color3.fromRGB(166, 226, 128),
        comment = Color3.fromRGB(150, 158, 168),
    ...
    local keywordSet = {
        ...
        ['return'] = true, ['then'] = true, ['true'] = true, ['type'] = true, ['typeof'] = true, ['until'] = true,
        ['while'] = true, ['as'] = true,
    }
    local typeSet = {
        ['any'] = true, ['boolean'] = true, ['buffer'] = true, ['nil'] = true, ['number'] = true, ['string'] = true,
        ['thread'] = true, ['unknown'] = true, ['never'] = true, ['userdata'] = true,
    }
    local globalSet = {
        ...
        table = true, task = true, debug = true, coroutine = true, os = true, utf8 = true, vector = true,
        pairs = true, ipairs = true, next = true, print = true, warn = true, error = true, assert = true,
        pcall = true, xpcall = true, select = true, rawequal = true, rawget = true, rawset = true, rawlen = true,
        setmetatable = true, getmetatable = true, loadstring = true, load = true, require = true,
        tonumber = true, tostring = true, type = true, typeof = true, unpack = true, wait = true,
        delay = true, spawn = true, tick = true, time = true, elapsedTime = true, gcinfo = true,
        collectgarbage = true, getfenv = true, setfenv = true, newproxy = true, settings = true, UserSettings = true,
    }
    ...
    local function buildHighlightLayers(source)
        local buffers = {
            plains = {},
            keywords = {}, globals = {}, strings = {}, comments = {}, numbers = {},
            funcs = {}, methods = {}, properties = {}, operators = {}, brackets = {},
        }
        ...
        local function appendPlain(text)
            for i = 1, #text do
                local rawCh = text:sub(i, i)
                local blankCh = blankFor(rawCh)
                for key, arr in pairs(buffers) do
                    arr[#arr + 1] = (key == 'plains') and rawCh or blankCh
                end
            end
        end
        ...
        while i <= n do
            ...
            if two == '--' and source:sub(i + 2, i + 2) == '[' then
            ...
            elseif ch:match('[%a_]') then
                ...
                if keywordSet[token] or typeSet[token] then
                ...
                elseif nextChar == '(' then
                    appendToLayer(globalSet[token] and 'globals' or 'funcs', token)
            ...
            elseif ch:match('[%[%]%(%){}]') then
                ...
                i += 1
            elseif three == '...' then
                appendToLayer('operators', three)
                i += 3
            elseif three == '//=' or three == '..=' then
                appendToLayer('operators', three)
                i += 3
            elseif two == '..' or two == '==' or two == '~=' or two == '<=' or two == '>=' or two == '::' or two == '//' or two == '->' then
                appendToLayer('operators', two)
                i += 2
            elseif ch:match('[%+%-%*/%%%^#=<>~:;,%.,|&%?]') then
    ...
    local function makeLayer(parent, name, color)
    ...
    Highlight.new = function(parent)
        ...
        local layers = {
            plains = makeLayer(content, 'Plain', colors.plain),
            keywords = makeLayer(content, 'Keywords', colors.keyword),
            globals = makeLayer(content, 'Globals', colors.global),
            strings = makeLayer(content, 'Strings', colors.string),
            comments = makeLayer(content, 'Comments', colors.comment),
            numbers = makeLayer(content, 'Numbers', colors.number),
            funcs = makeLayer(content, 'Functions', colors.func),
            methods = makeLayer(content, 'Methods', colors.method),
            properties = makeLayer(content, 'Properties', colors.property),
            operators = makeLayer(content, 'Operators', colors.operator),
            brackets = makeLayer(content, 'Brackets', colors.bracket),
        }
        ...
        local function measure(text)
            ...
            local lineHeight = TextService:GetTextSize('M', textBox.TextSize, textBox.Font, Vector2.new(1000, 1000)).Y + 2
            local width = math.max(parent.AbsoluteSize.X, math.min(longest, 2500) * 8 + 32)
            local height = math.max(parent.AbsoluteSize.Y, lines * lineHeight + 8)
            return width, height
        end

        local function syncSize(text)
            local width, height = measure(text)
            content.Size = UDim2.fromOffset(width, height)
            textBox.Size = UDim2.fromOffset(width - 8, height - 4)
        ...
        local function setLayerVisible(visible)
            for _, layer in pairs(layers) do
            ...
            textBox.TextTransparency = visible and 1 or 0
        end

        local function render(text)
            syncSize(text)
            if #text > syntaxMaxChars then
                setLayerVisible(false)
                return
            end
            setLayerVisible(true)
            local buffers = buildHighlightLayers(text)
            for name, layer in pairs(layers) do
                layer.Text = table.concat(buffers[name])
            end
        end

        textBox:GetPropertyChangedSignal('Text'):Connect(function()
            if not renderingText then
                rawString = textBox.Text or ''
            end
        end)
        parent:GetPropertyChangedSignal('AbsoluteSize'):Connect(function()
            syncSize(textBox.Text or '')
        end)

        return {
            setRaw = function(_, text)
                rawString = tostring(text or '')
                local display = normalizeDisplayText(rawString)
                if #rawString > displayMaxChars then
                    display = normalizeDisplayText(rawString:sub(1, displayMaxChars))
                        .. ('\n\n--[[ SimpleSpy kept the full %d character script for copy/execute, but stopped rendering here to avoid FPS drops. ]]'):format(#rawString)
                end
                renderingText = true
...
do
    ...
    ui.ResizeRight = Create('TextButton', {
        Parent = ui.Background,
        Active = true,
        AutoButtonColor = false,
        BackgroundTransparency = 1,
        ...
        Size = UDim2.new(0, 10, 1, -19),
        Text = '',
        ZIndex = 100000000,
    })
    ui.ResizeBottom = Create('TextButton', {
        Parent = ui.Background,
        Active = true,
        AutoButtonColor = false,
        BackgroundTransparency = 1,
    ...
    ui.ResizeBottomRight = Create('TextButton', {
        Parent = ui.Background,
        Active = true,
        AutoButtonColor = false,
        BackgroundTransparency = 1,
    ...
    ui.ResizeBottomLeft = Create('TextButton', {
        Parent = ui.Background,
        Active = true,
        AutoButtonColor = false,
        BackgroundTransparency = 1,
...
local function isIg()
    return ui.SimpleSpy3.IgnoreGuiInset == true
end
...
RuntimeState.ptr = function(input)
    if input and input.UserInputType == Enum.UserInputType.Touch and typeof(input.Position) == 'Vector3' then
...
RuntimeState.dragHit = function(input)
    local p = RuntimeState.ptr(input)
    if isInDragRange and isInDragRange(p) then
        return true
    end
    local inset = getInset()
    if inset.X ~= 0 or inset.Y ~= 0 then
        if isInDragRange and isInDragRange(p - inset) then
            return true
        end
        if isInDragRange and isInDragRange(p + inset) then
            return true
...
local incomingClientConns = {}
local descAddedConn
local descAddedInvokeConn
local oldIncomingNewIndex
local incomingHooksEnabled = false

local originalEvent = remoteEvent.FireServer
local originalFunction = remoteFunction.InvokeServer
local baseRemoteEvent = Instance.new('RemoteEvent')
local baseRemoteFunction = Instance.new('RemoteFunction')
local baseFireServer = baseRemoteEvent.FireServer
local baseInvokeServer = baseRemoteFunction.InvokeServer
baseRemoteEvent:Destroy()
baseRemoteFunction:Destroy()
local GetDebugIDInvoke = GetDebugIdHandler.Invoke
local internalRemotes = {}
...
realconfigs.autoStartSpy = rawget(getgenv(), 'SimpleSpyAutoStart')
if realconfigs.autoStartSpy == nil then
    realconfigs.autoStartSpy = true
else
    realconfigs.autoStartSpy = realconfigs.autoStartSpy == true
end
realconfigs.remoteScanBatchSize = math.max(
    tonumber(rawget(getgenv(), 'SimpleSpyRemoteScanBatchSize')) or 256,
    32
)
...
local weakReplacementPruneCounter = 0
local function isDeadTrackedInstance(inst)
    return typeof(inst) ~= 'Instance' or inst.Parent == nil
end

local function pruneTrackedRemoteMaps(force)
    weakReplacementPruneCounter += 1
    if not force and weakReplacementPruneCounter < 256 then
        return
    end
    weakReplacementPruneCounter = 0

    for remote, callback in pairs(_origClientInvoke) do
        if isDeadTrackedInstance(remote) then
            _origClientInvoke[remote] = nil
            wrappedClientInvoke[remote] = nil
            if type(callback) == 'function' and wrappedInvokeCallbacks[callback] == remote then
                wrappedInvokeCallbacks[callback] = nil
            end
        end
    end

    for callback, remote in pairs(wrappedInvokeCallbacks) do
        if type(callback) ~= 'function' or isDeadTrackedInstance(remote) then
            wrappedInvokeCallbacks[callback] = nil
        end
    end

    for callback, remote in pairs(logConnectionFunctions) do
        if type(callback) ~= 'function' or isDeadTrackedInstance(remote) then
            logConnectionFunctions[callback] = nil
        end
    end

    for remote in pairs(loggedClientEvent) do
        if isDeadTrackedInstance(remote) then
            loggedClientEvent[remote] = nil
        end
    end

    for remote in pairs(wrappedClientInvoke) do
        if isDeadTrackedInstance(remote) then
            wrappedClientInvoke[remote] = nil
        end
    end
...
caps.anyHookMetamethod = type(hookmetamethod) == 'function'
    or type(nativeHookMetamethod) == 'function'
...
caps.newIndexHook = caps.anyHookMetamethod
...
local function logthread(thread: thread)
    table.insert(RuntimeState.running_threads, thread)
end
...
function onBarInput(input)
    ...
    then
        local startPos = RuntimeState.ptr(input)
        local mainPos = ui.Background.Position
        local mainX = mainPos.X.Offset
        local mainY = mainPos.Y.Offset
        local currentPos = Vector2.new(mainX, mainY)
        if not RuntimeState.connections['drag'] then
            RuntimeState.connections['drag'] = RunService.RenderStepped:Connect(function()
                local newPos = RuntimeState.ptr(input)
                if newPos ~= startPos then
                    local delta = newPos - startPos
                    local currentX = mainX + delta.X
                    local currentY = mainY + delta.Y
                    local viewportSize =
                        SafeGetService('Workspace').CurrentCamera.ViewportSize
                    if currentX < 0 then
                        currentX = 0
                    elseif currentX > viewportSize.X - (sideClosed and 131 or ui.TopBar.AbsoluteSize.X) then
                        currentX = viewportSize.X - (sideClosed and 131 or ui.TopBar.AbsoluteSize.X)
                    end
                    if currentY < 0 then
                        currentY = 0
                    elseif currentY > viewportSize.Y - (closed and 19 or ui.Background.AbsoluteSize.Y) - getIY() then
                        currentY = viewportSize.Y - (closed and 19 or ui.Background.AbsoluteSize.Y) - getIY()
                    end
        ...
        table.insert(
            ...
            UserInputService.InputEnded:Connect(function(inputE)
                if input == inputE then
                    if RuntimeState.connections['drag'] then
                        RuntimeState.connections['drag']:Disconnect()
                        RuntimeState.connections['drag'] = nil
                    end
                end
            end)
        )
...
function toggleMinimize(override)
    ...
    if closed then
        ...
        RuntimeState.remotesFadeIn = fadeOut(ui.LeftPanel:QueryDescendants("Instance"))
        TweenService:Create(
            ui.LeftPanel,
            TweenInfo.new(0.5),
        ...
        wait(0.5)
        if RuntimeState.remotesFadeIn then
            RuntimeState.remotesFadeIn()
            RuntimeState.remotesFadeIn = nil
        end
        bringBackOnResize()
    end
    mainClosing = false
...
function toggleSideTray(override)
    ...
    if sideClosed then
        RuntimeState.rightFadeIn = fadeOut(ui.RightPanel:QueryDescendants("Instance"))
        wait(0.5)
        minimizeSize(0.5)
        wait(0.5)
        ...
        maximizeSize(0.5)
        wait(0.5)
        if RuntimeState.rightFadeIn then
            RuntimeState.rightFadeIn()
        end
        bringBackOnResize()
    end
    sideClosing = false
...
function toggleMaximize()
    if not sideClosed and not maximized then
        ...
        disable.MouseButton1Click:Connect(function()
            local p = getMP()
            if
                p.Y >= ui.CodeBox.AbsolutePosition.Y
                and p.Y <= ui.CodeBox.AbsolutePosition.Y + ui.CodeBox.AbsoluteSize.Y
                and p.X >= ui.CodeBox.AbsolutePosition.X
                and p.X <= ui.CodeBox.AbsolutePosition.X + ui.CodeBox.AbsoluteSize.X
            then
                return
            end
            TweenService:Create(
...
function isInResizeRange(p)
    ...
    if relativeP.X < -range or relativeP.Y < -range then
        return false
    end
    if relativeP.X > size.X + range or relativeP.Y > size.Y + range then
        return false
    end
    if relativeP.Y >= size.Y - range and relativeP.Y <= size.Y + range then
        if relativeP.X <= range then
            return true, 'BL'
        elseif relativeP.X >= size.X - range then
            return true, 'B'
        end
        return true, 'Y'
    elseif relativeP.X >= size.X - range and relativeP.X <= size.X + range then
        return true, 'X'
    end
    return false
end
...
function mouseEntered()
    local con = RuntimeState.connections['SIMPLESPY_CURSOR']
    if con then
        con:Disconnect()
        RuntimeState.connections['SIMPLESPY_CURSOR'] = nil
    end
    RuntimeState.connections['SIMPLESPY_CURSOR'] = RunService.RenderStepped:Connect(
        function()
            UserInputService.MouseIconEnabled = not RuntimeState.mouseInGui
            customCursor.Visible = RuntimeState.mouseInGui
            if RuntimeState.mouseInGui and getgenv().SimpleSpyExecuted then
                local mouseLocation = getMP()
                ...
                RuntimeState.connections['SIMPLESPY_CURSOR']:Disconnect()
            end
        end
    )
end
...
function mouseMoved()
    ...
    then
        if not RuntimeState.mouseInGui then
            RuntimeState.mouseInGui = true
            mouseEntered()
        end
    else
        RuntimeState.mouseInGui = false
    end
end
...
function validateSize()
    ...
    if y + ui.Background.AbsolutePosition.Y > maxY then
        if maxY - ui.Background.AbsolutePosition.Y >= 268 then
            y = maxY - ui.Background.AbsolutePosition.Y
        else
            y = 268
        end
    end
...
function startResize(input, mode)
    if
        input.UserInputType ~= Enum.UserInputType.MouseButton1
    ...
    if closed or RuntimeState.connections['SIMPLESPY_RESIZE'] then
        return
    end
    local startPos = RuntimeState.ptr(input)
    local startSize = ui.Background.AbsoluteSize
    local startFrame = ui.Background.Position
    local startX = startFrame.X.Offset
    local startY = startFrame.Y.Offset
    local startRight = startX + startSize.X
    RuntimeState.connections['SIMPLESPY_RESIZE'] = RunService.RenderStepped:Connect(function()
        local newPos = RuntimeState.ptr(input)
        if newPos == startPos then
            return
        end
        local delta = newPos - startPos
        local curX = startX
        local curW = startSize.X
        local curH = startSize.Y
        local canW = not sideClosed and not closed
        local canH = not closed
        if canW and (mode == 'X' or mode == 'B') then
            curW = startSize.X + delta.X
        elseif canW and mode == 'BL' then
            curX = startX + delta.X
            curW = startSize.X - delta.X
            if curX < 0 then
                curX = 0
        ...
        if canH and (mode == 'Y' or mode == 'B' or mode == 'BL') then
            curH = startSize.Y + delta.Y
        end
        if curW < 450 then
            curW = 450
            if mode == 'BL' then
                curX = startRight - curW
                if curX < 0 then
                    curX = 0
                end
            end
        ...
        ui.Background.Position = UDim2.new(0, curX, 0, startY)
        ui.Background.Size = UDim2.fromOffset(curW, curH)
        validateSize()
        if sideClosed then
            minimizeSize()
        else
    ...
    table.insert(
        ...
        UserInputService.InputEnded:Connect(function(inputE)
            if input == inputE then
                if RuntimeState.connections['SIMPLESPY_RESIZE'] then
                    RuntimeState.connections['SIMPLESPY_RESIZE']:Disconnect()
                    RuntimeState.connections['SIMPLESPY_RESIZE'] = nil
                end
            end
        end)
    )
...
function backgroundUserInput(input)
    local mousePos = RuntimeState.ptr(input)
    local inResizeRange, mode = isInResizeRange(mousePos)
    if not inResizeRange then
        local inset = getInset()
        if inset.X ~= 0 or inset.Y ~= 0 then
            inResizeRange, mode = isInResizeRange(mousePos - inset)
            if not inResizeRange then
                inResizeRange, mode = isInResizeRange(mousePos + inset)
            end
        end
    ...
    elseif RuntimeState.dragHit(input) then
        onBarInput(input)
    end
end
...
function eventSelect(frame)
    ...
    if selected and selected.Log then
        ...
        RuntimeState.codebox:setRaw(selected.GenScript)
    end
    if sideClosed then
        toggleSideTray()
    end
...
function makeToolTip(enable, text)
    if enable and text then
        if ui.ToolTip.Visible then
            ...
            local tooltip = RuntimeState.connections['ToolTip']
            if tooltip then
                tooltip:Disconnect()
            end
        end
        local first = true
        RuntimeState.connections['ToolTip'] = RunService.RenderStepped:Connect(function()
            local MousePos = getMP()
            ...
            local ViewportSizeY = ViewportSize.Y - getIY()
...
function newButton(name, description, onClick)
    ...
    return FunctionTemplate, Text, Button
end

function newTextBox(name, description, defaultText, onFocusLost)
    local FunctionTemplate = Create('Frame', {
        Name = name,
        Parent = ui.ScrollingFrame,
        BackgroundColor3 = Color3.new(1, 1, 1),
...
local function selectCall(log, idx)
    ...
    elseif rem:IsA("BindableEvent") then
        if m == "event" and firesignal then
            g ..= argsLen > 0
                and ("firesignal(%s.Event, unpack(args))"):format(r)
                or ("firesignal(%s.Event)"):format(r)
        else
            g ..= argsLen > 0
...
function genScript(remote, args, method)
    ...
    if #args > 0 then
        ...
        elseif remote:IsA("BindableEvent") then
            if method == "event" and firesignal then
                gen ..= ("firesignal(%s.Event, unpack(args))"):format(r)
            else
                gen ..= ("%s:Fire(unpack(args))"):format(r)
            end
        elseif remote:IsA("BindableFunction") then
            if method == "oninvoke" then
                gen ..= ("%s.OnInvoke = function(...)\n\treturn nil\nend"):format(r)
            else
        ...
        elseif remote:IsA("BindableEvent") then
            if method == "event" and firesignal then
                gen ..= ("firesignal(%s.Event)"):format(r)
            else
                gen ..= ("%s:Fire()"):format(r)
            end
        elseif remote:IsA("BindableFunction") then
...
local function packedLength(values)
    ...
    return RuntimeState.table_maxn(values)
end
...
logConnectionFunctions = logConnectionFunctions or {}
...
local function getIncomingSignal(inst)
    if inst:IsA('RemoteEvent') or inst:IsA('UnreliableRemoteEvent') then
        return inst.OnClientEvent, 'OnClientEvent'
    elseif inst:IsA('BindableEvent') then
        return inst.Event, 'Event'
    end
end

local function getInvokeCallbackMember(inst)
...
local function logClientEvent(re: Instance)
    pruneTrackedRemoteMaps(false)
    ...
    local signal, methodName = getIncomingSignal(re)
    if not signal then
        return
    end
    ...
    local function logConnection(...)
        ...
        if caps.getConnections then
            local ok, current = pcall(getconnections, signal)
    ...
    logConnectionFunctions[logConnection] = re
    local conn = signal:Connect(logConnection)
    incomingClientConns[re] = conn
end
...
local function wrapOnClientInvokeCallback(rf, callback)
    ...
    return function(...)
        ...
        local methodName = getInvokeCallbackMember(rf)
        local ok, results = pcall(function()
            return RuntimeState.table_pack(captureRemoteCall({
                remote = remoteRef,
                method = methodName,
                args = args,
                argsLength = argsLength,
        ...
        return unpack(results, 1, results.n or RuntimeState.table_maxn(results))
    end
end
...
local function wrapClientInvoke(rf)
    ...
    local callbackMember = getInvokeCallbackMember(rf)
    if not callbackMember then
        return
    end

    local function getExistingInvokeCallback()
        if getcallbackmember then
            local ok, cb = pcall(getcallbackmember, rf, callbackMember)
            if ok and type(cb) == 'function' then
                return cb
            end
        end
        local ok, cb = pcall(function()
            return rf[callbackMember]
        end)
        if ok and type(cb) == 'function' then
            return cb
        end
    ...
    then
        rf[callbackMember] = setWrappedOnClientInvoke(rf, current)
    end
end

local function disableIncomingHooks()
    incomingHooksEnabled = false
    configs.log_Receive = false

    for remote, conn in pairs(incomingClientConns) do
        pcall(function()
            conn:Disconnect()
        end)
        incomingClientConns[remote] = nil
    end

    for remote, callback in pairs(_origClientInvoke) do
        if typeof(remote) == 'Instance' and type(callback) == 'function' then
            pcall(function()
                local callbackMember = getInvokeCallbackMember(remote)
                if callbackMember then
                    remote[callbackMember] = callback
                end
            end)
        end
    end
    ...
    if oldIncomingNewIndex and caps.newIndexHook and type(hookmetamethod) == 'function' then
        pcall(hookmetamethod, game, '__newindex', oldIncomingNewIndex)
        oldIncomingNewIndex = nil
    end

    clear(loggedClientEvent)
    clear(logConnectionFunctions)
    clear(_origClientInvoke)
    clear(wrappedClientInvoke)
    clear(wrappedInvokeCallbacks)
end

local function trackIncomingRemote(inst)
    if
    ...
    then
        logClientEvent(inst)
    elseif inst:IsA('RemoteFunction') or inst:IsA('BindableFunction') then
        wrapClientInvoke(inst)
    end
end

local function scanIncomingRemotes()
    local queue = game:GetChildren()
    local index = 1
    local processed = 0

    while incomingHooksEnabled and index <= #queue do
        local inst = queue[index]
        index += 1

        trackIncomingRemote(inst)
        ...
        if ok and children then
            ...
            for childIndex = 1, count do
                queue[offset + childIndex] = children[childIndex]
            end
        end

        processed += 1
        if processed % realconfigs.remoteScanBatchSize == 0 then
            task.wait()
        end
    end
end
...
local function enableIncomingHooks()
    if incomingHooksEnabled then
        configs.log_Receive = true
        return
    end

    incomingHooksEnabled = true
    configs.log_Receive = true

    if caps.newIndexHook and type(hookmetamethod) == 'function' and not oldIncomingNewIndex then
        local ok, hooked = pcall(
            ...
            function(self, k, v)
                if
                    incomingHooksEnabled
                    and not safeCheckCaller()
                    and typeof(self) == 'Instance'
                    and (
                        (self:IsA('RemoteFunction') and k == 'OnClientInvoke')
                        or (self:IsA('BindableFunction') and k == 'OnInvoke')
                    )
                    and type(v) == 'function'
                then
                    return oldIncomingNewIndex(self, k, setWrappedOnClientInvoke(self, v))
                end
                return oldIncomingNewIndex(self, k, v)
            end
        )
        if ok then
            oldIncomingNewIndex = hooked
        end
    end

    if not descAddedConn then
        descAddedConn = game.DescendantAdded:Connect(function(inst)
            if
                incomingHooksEnabled
                and (
                    inst:IsA('RemoteEvent')
    ...
    if not descAddedInvokeConn then
        descAddedInvokeConn = game.DescendantAdded:Connect(function(inst)
            if
                incomingHooksEnabled
                and (inst:IsA('RemoteFunction') or inst:IsA('BindableFunction'))
            then
                wrapClientInvoke(inst)
            end
        end)
    ...
    logthread(spawn(scanIncomingRemotes))
end
...
local typeofv2sfunctions = {
    ...
    Instance = function(v)
        ...
        return i2p(v, RuntimeState.generation[DebugId])
    end,
...
RuntimeState.formatstr_with_status = function(s, indentation)
...
function formatstr(s, indentation)
    local literal = RuntimeState.formatstr_with_status(s, indentation)
    return literal
end

--- Adds \'s to the text as a replacement to whitespace chars and other things because string.format can't yayeet

RuntimeState.isFinished = function(coroutines: table)
...
function handlespecials(s, indentation)
    ...
    repeat
        ...
        if byte(char) then
            ...
            local specialfunc = RuntimeState.specialstrings[char]
    ...
    while not RuntimeState.isFinished(coroutines) do
...
RuntimeState._buf_bytes = function(b)
...
RuntimeState.buffer_hex_preview = function(bytes, limit)
    local hexdumpLines = {}
    local lineBytes = {}
...
RuntimeState.buffer_string_literal = function(bytes, maxLiteralBytes, indentation)
    local count = #bytes
    ...
    local literal, reachedMax =
        RuntimeState.formatstr_with_status(table.concat(chars), indentation or 0)
    if reachedMax then
        return nil
    end
...
RuntimeState.buffer_char_literal = function(bytes)
    local count = #bytes
    if count == 0 then
...
function buffer_literal(u, indentation)
    ...
    local bytes = RuntimeState._buf_bytes(u)
    local previewLimit =
    ...
    if bufferDumpEnabled and previewLimit > 0 then
        hexdump, shown = RuntimeState.buffer_hex_preview(bytes, previewLimit)
    end
    ...
    local literal = RuntimeState.buffer_string_literal(bytes, maxLiteralBytes, indentation)
    if not literal then
        literal = RuntimeState.buffer_char_literal(bytes)
    end
    if hexdump then
...
RuntimeState.scheduleUrgent = function(f, ...)
...
RuntimeState.taskscheduler = function()
    if not toggle then
        scheduled = {}
        return
    end
...
RuntimeState.tablecheck = function(tabletocheck, instance, id)
...
function remoteHandler(data)
    ...
    if configs.autoblock then
        local id = data.id
        if RuntimeState.excluding[id] then
            return
        end

        local now = tick()
        local h = RuntimeState.history[id]

        if not h then
            h = { badOccurances = 0, lastCall = now }
            RuntimeState.history[id] = h
        else
            if now - h.lastCall < 1 then
                h.badOccurances += 1
                h.lastCall = now
                if h.badOccurances >= 3 then
                    RuntimeState.excluding[id] = true
                    return
                end
            else
                h.badOccurances = 0
...
RuntimeState.table_maxn = table.maxn
if not RuntimeState.table_maxn then
    RuntimeState.table_maxn = function(t)
        local max = 0
        for k in next, t do
...
RuntimeState.normalizeMethod = function(method)
    if typeof(method) == 'string' then
        return lower(method)
    end
end

RuntimeState.methodAllowed = function(classData, transferType, method)
...
RuntimeState.callOriginal = function(callback, args, length)
    ...
    if not callLength then
        callLength = RuntimeState.table_maxn(callArgs)
    end
...
RuntimeState.table_pack = table.pack
if not RuntimeState.table_pack then
    RuntimeState.table_pack = function(...)
        local packed = { ... }
        packed.n = select('#', ...)
        return packed
    end
...
SimpleSpy._withThreadIdentity = function(level, callback, ...)
    if type(callback) ~= 'function' then
        return
    end

    if type(get_thread_identity) ~= 'function' or type(set_thread_identity) ~= 'function' then
        return callback(...)
    end

    local previousIdentity
    local hasIdentity = pcall(function()
        previousIdentity = get_thread_identity()
    end)

    pcall(set_thread_identity, (level or 8) :: number)

    local results = RuntimeState.table_pack(pcall(callback, ...))

    if hasIdentity then
        pcall(set_thread_identity, previousIdentity)
    end
...
captureRemoteCall = function(options)
    ...
    if not argsLength then
        argsLength = RuntimeState.table_maxn(args)
    end
    local remote = options.remote
    local transferType = options.transferType == 'Receive' and 'Receive'
        or 'Send'
    if internalRemotes[remote] then
        return RuntimeState.callOriginal(options.callOriginal, args, argsLength)
    end
    if typeof(remote) ~= 'Instance' then
        return RuntimeState.callOriginal(options.callOriginal, args, argsLength)
    end

    local classData = RuntimeState.RemoteClassData[remote.ClassName]
    local normalizedMethod = RuntimeState.normalizeMethod(options.method)
    if not RuntimeState.methodAllowed(classData, transferType, normalizedMethod) then
        return RuntimeState.callOriginal(options.callOriginal, args, argsLength)
    end
    ...
    if transferType == 'Send' then
        blockcheck = RuntimeState.tablecheck(blocklist, remote, id)
    ...
    if
        ...
        and RuntimeState.tablecheck(blocklist, remote, id)
    then
        return
    end
    ...
    local blacklisted = RuntimeState.tablecheck(blacklist, remote, id)
    local entry
    ...
    if argsLength and argsLength > 0 then
        results = RuntimeState.table_pack(options.callOriginal(unpack(args, 1, argsLength)))
    else
        results = RuntimeState.table_pack(options.callOriginal())
    end
    local resultLength = (results and results.n) or RuntimeState.table_maxn(results)
...
RuntimeState.call_original_namecall = function(method, self, ...)
    if setnamecallmethod then
        setnamecallmethod(method)
    end
    return RuntimeState.originalnamecall(self, ...)
end

RuntimeState.getCallingInfo = function(what)
    local baseLevel = synv3 and 2 or 4

    for level = baseLevel, 10 do
        local result = safeDebugInfo(level, what)
        if result == nil then
            break
        end
        if what == 's' and result == '[C]' then
            continue
        end
        return result
    end

    return safeDebugInfo(baseLevel, what)
end

RuntimeState.cloneHook = function(fn)
...
RuntimeState.wrapHook = function(fn)
...
RuntimeState.newnamecall = function(self, ...)
    local method = caps.getNamecallMethod and getnamecallmethod() or nil
    local normalizedMethod = RuntimeState.normalizeMethod(method)
    if
        normalizedMethod ~= 'fireserver'
        and normalizedMethod ~= 'invokeserver'
        and normalizedMethod ~= 'fire'
        and normalizedMethod ~= 'invoke'
    then
        return RuntimeState.call_original_namecall(method, self, ...)
    end
    local args = { ... }
    local argsLength = select('#', ...)
    ...
    return captureRemoteCall({
        ...
        callingscript = getcallingscript(),
        infofunc = RuntimeState.getCallingInfo('f'),
        infoline = RuntimeState.getCallingInfo('l'),
        infosource = RuntimeState.getCallingInfo('s'),
...
RuntimeState.newFire = RuntimeState.wrapHook(function(self, ...)
    local args = { ... }
    local argsLength = select('#', ...)
    if internalRemotes[self] then
...
RuntimeState.newInvoke = function(self, ...)
    local args = { ... }
    local argsLength = select('#', ...)
    if internalRemotes[self] then
...
RuntimeState.disablehooks = function()
    if not RuntimeState.hookState.active then
        return
    end
    RuntimeState.hookState.active = false

    SimpleSpy._withThreadIdentity(8, function()
        if synv3 then
            if RuntimeState.hookState.namecall and caps.unhookAPI and caps.rawMetatable and RuntimeState.originalnamecall then
                pcall(unhook, getrawmetatable(game).__namecall, RuntimeState.originalnamecall)
            end
            if RuntimeState.hookState.fireServer and caps.unhookAPI and originalEvent then
                pcall(unhook, baseFireServer, originalEvent)
            end
            if RuntimeState.hookState.invokeServer and caps.unhookAPI and originalFunction then
                pcall(unhook, baseInvokeServer, originalFunction)
            end
            if RuntimeState.hookState.namecall and caps.restoreFunction and RuntimeState.originalnamecall then
                pcall(restorefunction, RuntimeState.originalnamecall)
            end
            if RuntimeState.hookState.fireServer and caps.restoreFunction and originalEvent then
                pcall(restorefunction, originalEvent)
            end
            if RuntimeState.hookState.invokeServer and caps.restoreFunction and originalFunction then
                pcall(restorefunction, originalFunction)
            end
        else
            if RuntimeState.hookState.namecall and RuntimeState.originalnamecall then
                if type(hookmetamethod) == 'function' then
                    pcall(hookmetamethod, game, '__namecall', RuntimeState.originalnamecall)
                elseif caps.hookFunction and caps.rawMetatable then
                    pcall(
                        hookfunction,
                        getrawmetatable(game).__namecall,
                        RuntimeState.originalnamecall
                    )
                end
            end
            if RuntimeState.hookState.fireServer and caps.hookFunction and originalEvent then
                pcall(hookfunction, baseFireServer, originalEvent)
            end
            if RuntimeState.hookState.invokeServer and caps.hookFunction and originalFunction then
                pcall(hookfunction, baseInvokeServer, originalFunction)
            end
        end
    end)

    RuntimeState.hookState.namecall = false
    RuntimeState.hookState.fireServer = false
    RuntimeState.hookState.invokeServer = false
end
...
function toggleSpy()
    ...
    if not toggle then
        SimpleSpy._withThreadIdentity(8, function()
            ...
            if synv3 then
                if caps.namecallHook and caps.hookAPI and caps.rawMetatable then
                    local ok, previous = pcall(
                        hook,
                        getrawmetatable(game).__namecall,
                        RuntimeState.cloneHook(RuntimeState.newnamecall)
                    )
                    if ok then
                        oldnamecall = previous
                        RuntimeState.hookState.namecall = true
                    end
                end
                if caps.hookAPI then
                    local okFire, oldFire = pcall(
                        hook,
                        baseFireServer,
                        RuntimeState.cloneHook(RuntimeState.newFire)
                    )
                    if okFire and oldFire then
                        originalEvent = oldFire
                        RuntimeState.hookState.fireServer = true
                    end
                    local okInvoke, oldInvoke = pcall(
                        hook,
                        baseInvokeServer,
                        RuntimeState.cloneHook(RuntimeState.newInvoke)
                    )
                    if okInvoke and oldInvoke then
                        originalFunction = oldInvoke
                        RuntimeState.hookState.invokeServer = true
                    end
                end
            else
            ...
            if oldnamecall then
                RuntimeState.originalnamecall = RuntimeState.originalnamecall
                    or function(...)
                        return oldnamecall(...)
                    end
            end
            RuntimeState.hookState.active = RuntimeState.hookState.namecall
                or RuntimeState.hookState.fireServer
                or RuntimeState.hookState.invokeServer
        end)
    else
        RuntimeState.disablehooks()
    end
...
RuntimeState.shutdown = function()
    if RuntimeState.schedulerconnect then
        RuntimeState.schedulerconnect:Disconnect()
    end
    for _, connection in next, RuntimeState.connections do
        connection:Disconnect()
    end
    for i, v in next, RuntimeState.running_threads do
        if ThreadIsNotDead(v) then
            close(v)
        end
    end
    clear(RuntimeState.running_threads)
    clear(RuntimeState.connections)
    clear(logs)
    clear(remoteLogs)
    for remote, callback in pairs(_origClientInvoke) do
        if typeof(remote) == 'Instance' and type(callback) == 'function' then
...
RuntimeState.initializeSimpleSpy = function()
    if not RunService:IsClient() then
        error('SimpleSpy cannot run on the server!')
    end
    getgenv().SimpleSpyShutdown = RuntimeState.shutdown
    if realconfigs.autoStartSpy then
        onToggleButtonClick()
    else
        ui.Simple.TextColor3 = Color3.fromRGB(252, 51, 51)
    end
    if not caps.sendHooks then
        ErrorPrompt(
            'Outbound hooks are unavailable.',
            true
        )
    ...
    RuntimeState.codebox = Highlight.new(ui.CodeBox)
    logthread(spawn(function()
        ...
        RuntimeState.codebox:setRaw((suc and err) or '')
    end))
    ...
    ui.TopBar.InputBegan:Connect(function(input)
        if RuntimeState.dragHit(input) then
            onBarInput(input)
        end
    end)
    ui.Simple.InputBegan:Connect(function(input)
        if RuntimeState.dragHit(input) then
            onBarInput(input)
        end
    end)
    ...
    ui.ResizeBottomRight.InputBegan:Connect(function(input)
        startResize(input, 'B')
    end)
    ui.ResizeBottomLeft.InputBegan:Connect(function(input)
        startResize(input, 'BL')
    end)
    ...
    ui.CloseButton.MouseButton1Click:Connect(RuntimeState.shutdown)
    table.insert(
        RuntimeState.connections,
    ...
    RuntimeState.schedulerconnect = RunService.Heartbeat:Connect(RuntimeState.taskscheduler)
    protectUI(ui.SimpleSpy3)
    bringBackOnResize()
    table.insert(
        RuntimeState.connections,
        ui.SimpleSpy3:GetPropertyChangedSignal('IgnoreGuiInset'):Connect(bringBackOnResize)
    )
    logthread(spawn(function()
...
if not getgenv().SimpleSpyExecuted then
    RuntimeState.initResult = RuntimeState.table_pack(pcall(RuntimeState.initializeSimpleSpy))
    if RuntimeState.initResult[1] then
        getgenv().SimpleSpyExecuted = true
    else
        RuntimeState.shutdown()
        ErrorPrompt('An error has occured:\n' .. rawtostring(RuntimeState.initResult[2]))
        return
    end
else
    ui.SimpleSpy3:Destroy()
...
do
    ...
    ui.NextButton.InputBegan:Connect(function(input)
        if
            ...
            or input.UserInputType == Enum.UserInputType.Touch
        then
            local startMouse = RuntimeState.ptr(input)
            local startPos = ui.NextButton.Position
            if not RuntimeState.connections['dragNextb'] then
                RuntimeState.connections['dragNextb'] = RunService.RenderStepped:Connect(
                    function()
                        local newPos = RuntimeState.ptr(input)
                        if newPos ~= startMouse then
                            local delta = newPos - startMouse
                            ui.NextButton.Position = UDim2.new(
                                startPos.X.Scale,
            ...
            table.insert(
                ...
                UserInputService.InputEnded:Connect(function(e)
                    if e == input and RuntimeState.connections['dragNextb'] then
                        RuntimeState.connections['dragNextb']:Disconnect()
                        RuntimeState.connections['dragNextb'] = nil
                    end
                end)
            )
        end
...
newButton('Copy Code', function()
    ...
    setclipboard(RuntimeState.codebox:getString())
...
newButton('Run Code', function()
    ...
    local src = RuntimeState.codebox:getString()
...
RuntimeState.spamRemote.selectedBlocked = function(remote)
    if not selected or not remote then
        return false
    end
    return blocklist[OldDebugId(remote)] == true or blocklist[selected.Name] == true
end

RuntimeState.spamRemote.runSelected = function()
    ...
    if RuntimeState.spamRemote.selectedBlocked(remote) then
        ui.TextLabel.Text = 'Selected remote is blocked'
        return false
    end

    local args = selected.args or {}
    local n = args.n or #args
    local method = tostring(selected.method or ''):lower()

    local ok, err = pcall(function()
        if remote:IsA('RemoteEvent') or remote:IsA('UnreliableRemoteEvent') then
            if method == 'onclientevent' then
                if firesignal then
                    firesignal(remote.OnClientEvent, unpack(args, 1, n))
                end
            else
        ...
        elseif remote:IsA('BindableEvent') then
            if method == 'event' and firesignal then
                firesignal(remote.Event, unpack(args, 1, n))
            else
                remote:Fire(unpack(args, 1, n))
            end
        elseif remote:IsA('BindableFunction') and method ~= 'oninvoke' then
            remote:Invoke(unpack(args, 1, n))
        end
    end)

    if not ok then
        ui.TextLabel.Text = 'Spam error: ' .. tostring(err)
        return false
    end
    return true
end

RuntimeState.spamRemote.refreshLabels = function()
    if RuntimeState.spamRemote.label then
        RuntimeState.spamRemote.label.Text = ('Spam Remote: %s'):format(RuntimeState.spamRemote.enabled and 'ON' or 'OFF')
    end
    if RuntimeState.spamRemote.modeLabel then
        RuntimeState.spamRemote.modeLabel.Text = ('Spam Mode: %s'):format(RuntimeState.spamRemote.modes[RuntimeState.spamRemote.modeIndex])
    end
    if RuntimeState.spamRemote.delayBox then
        RuntimeState.spamRemote.delayBox.Text = ('%g'):format(RuntimeState.spamRemote.delay)
    end
end

RuntimeState.spamRemote.disconnect = function()
    local connection = RuntimeState.connections['SpamRemote']
    if connection then
        connection:Disconnect()
        RuntimeState.connections['SpamRemote'] = nil
    end
end

RuntimeState.spamRemote.connect = function()
    RuntimeState.spamRemote.disconnect()
    RuntimeState.spamRemote.elapsed = 0

    local mode = RuntimeState.spamRemote.modes[RuntimeState.spamRemote.modeIndex]
    local signal = RunService[mode] or RunService.Heartbeat
    RuntimeState.connections['SpamRemote'] = signal:Connect(function(dt)
        if not RuntimeState.spamRemote.enabled then
            return
        end

        if RuntimeState.spamRemote.delay <= 0 then
            RuntimeState.spamRemote.runSelected()
            return
        end

        RuntimeState.spamRemote.elapsed = RuntimeState.spamRemote.elapsed + (dt or 0)
        if RuntimeState.spamRemote.elapsed >= RuntimeState.spamRemote.delay then
            RuntimeState.spamRemote.elapsed = 0
            RuntimeState.spamRemote.runSelected()
        end
    end)
...
RuntimeState.spamRemote.setEnabled = function(enabled)
    RuntimeState.spamRemote.enabled = enabled == true
    if RuntimeState.spamRemote.enabled then
        RuntimeState.spamRemote.connect()
    else
        RuntimeState.spamRemote.disconnect()
    end
    RuntimeState.spamRemote.refreshLabels()
    ui.TextLabel.Text = ('Spam Remote: %s'):format(RuntimeState.spamRemote.enabled and 'ON' or 'OFF')
end

RuntimeState.spamRemote.label = select(2, newButton('Spam Remote: OFF', function()
    return ('[%s] Run the selected remote repeatedly on %s'):format(
        RuntimeState.spamRemote.enabled and 'ON' or 'OFF',
        RuntimeState.spamRemote.modes[RuntimeState.spamRemote.modeIndex]
    )
end, function()
    RuntimeState.spamRemote.setEnabled(not RuntimeState.spamRemote.enabled)
end))

RuntimeState.spamRemote.delayBox = select(2, newTextBox('Spam Delay', function()
    return 'Delay between spam calls, in seconds. Use 0 to run every signal step.'
end, '0', function(box)
    local value = tonumber(box.Text) or 0
    if value < 0 then
        value = 0
    end
    RuntimeState.spamRemote.delay = value
    RuntimeState.spamRemote.elapsed = 0
    RuntimeState.spamRemote.refreshLabels()
    ui.TextLabel.Text = ('Spam delay: %g seconds'):format(RuntimeState.spamRemote.delay)
end))

RuntimeState.spamRemote.modeLabel = select(2, newButton('Spam Mode: PreSimulation', function()
    return 'Click to cycle spam mode: RenderStepped, PreSimulation, Heartbeat'
end, function()
    RuntimeState.spamRemote.modeIndex += 1
    if RuntimeState.spamRemote.modeIndex > #RuntimeState.spamRemote.modes then
        RuntimeState.spamRemote.modeIndex = 1
    end
    if RuntimeState.spamRemote.enabled then
        RuntimeState.spamRemote.connect()
    ...
    RuntimeState.spamRemote.refreshLabels()
    ui.TextLabel.Text = ('Spam mode: %s'):format(RuntimeState.spamRemote.modes[RuntimeState.spamRemote.modeIndex])
end))
RuntimeState.spamRemote.refreshLabels()
...
newButton('Log Client', function()
    return ('[%s] Incoming events/invokes'):format(
        configs.log_Receive and 'ON' or 'OFF'
    )
end, function()
    if configs.log_Receive then
        disableIncomingHooks()
        ui.TextLabel.Text = 'Incoming remote logging disabled'
    else
        enableIncomingHooks()
        ui.TextLabel.Text = 'Incoming remote logging enabled'
    end
end)
...
newButton('Function Info', function()
    ...
    if func then
        ...
        if typeoffunc ~= 'string' then
            RuntimeState.codebox:setRaw('--[[Generating Function Info please wait]]')
            RunService.Heartbeat:Wait()
            ...
            RuntimeState.codebox:setRaw('--[[Converting table to string please wait]]')
            selected.Function = v2v({ functionInfo = info })
        end
        RuntimeState.codebox:setRaw(
...
if caps.decompile then
    newButton('Decompile', function()
        ...
        if selected and selected.Source then
            ...
            if not RuntimeState.DecompiledScripts[Source] then
                RuntimeState.codebox:setRaw('--[[Decompiling]]')
                ...
                xpcall(function()
                    ...
                    if (decompiledsource):find('script') and Sourcev2s then
                        RuntimeState.DecompiledScripts[Source] = ('local script = %s\n%s'):format(
                            Sourcev2s,
                            decompiledsource
                        )
                    end
                end, function(err)
                    return RuntimeState.codebox:setRaw(
                        ('--[[\nAn error has occured\n%s\n]]'):format(err)
                    )
                end)
            end
            RuntimeState.codebox:setRaw(RuntimeState.DecompiledScripts[Source] or '--No Source Found')
            ui.TextLabel.Text = 'Done!'
        else
            ...
            if selected then
                ...
                if Remote and Remote:IsA("RemoteFunction") then
                    if selected.returnvalue and selected.returnvalue.data then
                        return RuntimeState.codebox:setRaw(v2s(selected.returnvalue.data))
                    end
                    return RuntimeState.codebox:setRaw("No data was returned")
                else
                    RuntimeState.codebox:setRaw("RemoteFunction expected got "..(Remote and Remote.ClassName))
                end
            end
        end
    )]]
...
newButton('Path Mode', function()
    local mode = configs.pathMode or 1
    return ('Path style: %s'):format(SimpleSpy._pathModeNames[mode] or '?')
end, function()
    local mode = configs.pathMode or 1
    mode += 1
    if mode > 3 then
    ...
    ui.TextLabel.Text = ('Path style set to %s'):format(SimpleSpy._pathModeNames[mode])
end)
