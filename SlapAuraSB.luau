...
if type(runtimeEnv.__SlapAuraSBRuntime) == "table" and type(runtimeEnv.__SlapAuraSBRuntime.cleanup) == "function" then
	pcall(runtimeEnv.__SlapAuraSBRuntime.cleanup);
end;
local runtimeState = {
	alive = true,
	connections = {}
};
runtimeEnv.__SlapAuraSBRuntime = runtimeState;
local function trackConn(conn)
	if conn then
		runtimeState.connections[#runtimeState.connections + 1] = conn;
	end;
	return conn;
end;
...
local function removeRemote(v)
	if attackRemoteSet[v] then
		...
		for i = #attackRemotes, 1, -1 do
			if attackRemotes[i] == v then
				table.remove(attackRemotes, i);
			end;
		end;
	end;
end;
for _, d in ipairs(__lt.cm("ReplicatedStorage", "QueryDescendants", "Instance")) do
	addRemote(d);
end;
trackConn(ReplicatedStorage.DescendantAdded:Connect(addRemote));
trackConn(ReplicatedStorage.DescendantRemoving:Connect(removeRemote));
local ghostRemote = __lt.cm("ReplicatedStorage", "WaitForChild", "Ghostinvisibilityactivated");
...
local function initPlayer(p)
	...
	local function setFlag(f)
		...
		if f then
			trackConn(f.Changed:Connect(function()
				playersData[p].inArena = f.Value;
			end));
		end;
	end;
	trackConn(p.CharacterAdded:Connect(function(ch)
		rootCache[ch] = nil;
		local f = ch:FindFirstChild("isInArena") or ch:FindFirstChild("IsInDefaultArena");
		setFlag(f);
	end));
	trackConn(p.CharacterRemoving:Connect(function(ch)
		rootCache[ch] = nil;
	end));
	if p.Character then
...
trackConn(Players.PlayerAdded:Connect(initPlayer));
...
trackConn(localPlayer.CharacterRemoving:Connect(onCharacterRemoving));
if localPlayer.Character then
	onCharacterAdded(localPlayer.Character);
end;
local guiParent = type(gethui) == "function" and gethui() or __lt.cs("CoreGui", cloneref) or localPlayer:WaitForChild("PlayerGui");
local screenGui = Instance.new("ScreenGui");
screenGui.Name = "Sex Hub";
...
runtimeState.cleanup = function()
	...
	auraEnabled = false;
	for _, conn in ipairs(runtimeState.connections) do
		pcall(function()
			conn:Disconnect();
		end);
	end;
	...
	if runtimeEnv.__SlapAuraSBRuntime == runtimeState then
		runtimeEnv.__SlapAuraSBRuntime = nil;
	end;
end;
...
local function showToast(text, dur)
	...
	s.Thickness = 1;
	s.Color = Color3.fromRGB(86, 142, 255);
	s.Transparency = 0.5;
	s.Parent = f;
	...
	s.Transparency = 1;
	(__lt.cm("TweenService", "Create", f, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		Position = UDim2.new(0.5, 0, 1, 0),
		BackgroundTransparency = 0.15
	})):Play();
	(__lt.cm("TweenService", "Create", l, TweenInfo.new(0.18), {
		TextTransparency = 0
	})):Play();
	(__lt.cm("TweenService", "Create", s, TweenInfo.new(0.18), {
		Transparency = 0.5
	})):Play();
	task.delay(dur or 2.5, function()
		local t1 = __lt.cm("TweenService", "Create", f, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
			Position = UDim2.new(0.5, 0, 1, 20),
			BackgroundTransparency = 1
		});
		local t2 = __lt.cm("TweenService", "Create", l, TweenInfo.new(0.16), {
			TextTransparency = 1
		});
		local t3 = __lt.cm("TweenService", "Create", s, TweenInfo.new(0.16), {
			Transparency = 1
		});
		t1:Play();
		t2:Play();
...
local function ripple(parent)
	local m = __lt.cm("UserInputService", "GetMouseLocation");
	...
	(__lt.cm("TweenService", "Create", r, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
...
local function button(text, parent, w, h)
	...
	bs.Thickness = 1;
	bs.Color = Color3.fromRGB(86, 142, 255);
	bs.Transparency = 0.25;
	bs.Parent = b;
	table.insert(rgbStrokes, bs);
	b.MouseEnter:Connect(function()
		(__lt.cm("TweenService", "Create", b, TweenInfo.new(0.14), {
			BackgroundColor3 = Color3.fromRGB(40, 40, 55)
		})):Play();
		(__lt.cm("TweenService", "Create", b, TweenInfo.new(0.14), {
			Size = UDim2.new(0, (w or 160) + 2, 0, h or 36)
		})):Play();
	end);
	b.MouseLeave:Connect(function()
		(__lt.cm("TweenService", "Create", b, TweenInfo.new(0.16), {
			BackgroundColor3 = Color3.fromRGB(32, 32, 42)
		})):Play();
		(__lt.cm("TweenService", "Create", b, TweenInfo.new(0.16), {
			Size = UDim2.new(0, w or 160, 0, h or 36)
		})):Play();
	end);
...
tabStroke.Color = Color3.fromRGB(86, 142, 255);
tabStroke.Transparency = 0.25;
...
local tabSelCorner = Instance.new("UICorner");
tabSelCorner.CornerRadius = UDim.new(0, 12);
tabSelCorner.Parent = tabSelector;
...
local function moveTabSelector(toBtn, animate)
	...
	if animate then
		(__lt.cm("TweenService", "Create", tabSelector, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
	...
	for _, b in ipairs({
		...
		(__lt.cm("TweenService", "Create", b, TweenInfo.new(0.12), {
			TextColor3 = b == toBtn and Color3.fromRGB(255, 255, 255) or Color3.fromRGB(220, 220, 235)
		})):Play();
	end;
end;
...
tabActions.MouseButton1Click:Connect(function()
	ripple(tabBg);
	moveTabSelector(tabActions, true);
	showPage(pageActions);
end);
tabSettings.MouseButton1Click:Connect(function()
	ripple(tabBg);
	moveTabSelector(tabSettings, true);
	showPage(pageSettings);
end);
tabCombat.MouseButton1Click:Connect(function()
	ripple(tabBg);
	moveTabSelector(tabCombat, true);
	showPage(pageCombat);
end);
task.defer(function()
	moveTabSelector(tabActions, false);
	showPage(pageActions);
end);
...
local function toggleControl(parent, labelText, initial)
	...
	ts.Thickness = 1;
	ts.Color = Color3.fromRGB(86, 142, 255);
	ts.Transparency = 0.2;
	ts.Parent = t;
	...
	local function setState(v)
		...
		(__lt.cm("TweenService", "Create", t, TweenInfo.new(0.18, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			BackgroundColor3 = goalBg
		})):Play();
		(__lt.cm("TweenService", "Create", knob, TweenInfo.new(0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			Position = goalPos
		})):Play();
	end;
...
local function slider(parent, labelText, minV, maxV, startV, decimals)
	...
	thS.Thickness = 1;
	thS.Color = Color3.fromRGB(190, 190, 210);
	thS.Transparency = 0.1;
	thS.Parent = thumb;
	...
	local function setFromAlpha(a, tween)
		...
		if tween then
			(__lt.cm("TweenService", "Create", fill, ti, {
				Size = UDim2.new(a, 0, 1, 0)
			})):Play();
			(__lt.cm("TweenService", "Create", thumb, ti, {
	...
	return {
		...
		Set = function(v)
			...
			setFromAlpha((v - minV) / (maxV - minV), true);
		end
	};
end;
...
segCorner.CornerRadius = UDim.new(0, 12);
segCorner.Parent = seg;
local segStroke = Instance.new("UIStroke");
segStroke.Thickness = 1;
segStroke.Color = Color3.fromRGB(86, 142, 255);
segStroke.Transparency = 0.25;
...
local function moveModeSelector(toBtn, animate)
	...
	local w = toBtn.AbsoluteSize.X;
	if animate then
		(__lt.cm("TweenService", "Create", selector, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Position = UDim2.new(0, relX, 0, 0),
			Size = UDim2.new(0, w, 1, 0)
		})):Play();
	else
...
task.defer(function()
	moveModeSelector(enterBtn, false);
end);
trackConn(RunService.RenderStepped:Connect(function()
...
miniStroke.Color = Color3.fromRGB(86, 142, 255);
...
local function minimizeToCircle()
	...
	dockButton.ImageTransparency = 1;
	(__lt.cm("TweenService", "Create", panel, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
		BackgroundTransparency = 1
	})):Play();
	(__lt.cm("TweenService", "Create", panelShadow, TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.In), {
		ImageTransparency = 1
	})):Play();
	(__lt.cm("TweenService", "Create", container, TweenInfo.new(0.22, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
		Size = UDim2.new(0, 64, 0, 64)
	})):Play();
	task.delay(0.22, function()
		panel.Visible = false;
		panelShadow.Visible = false;
		(__lt.cm("TweenService", "Create", miniCircle, TweenInfo.new(0.24, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
			Size = UDim2.new(0, 56, 0, 56)
		})):Play();
		(__lt.cm("TweenService", "Create", dockButton, TweenInfo.new(0.24), {
...
local function restoreFromCircle()
	...
	(__lt.cm("TweenService", "Create", dockButton, TweenInfo.new(0.16), {
		ImageTransparency = 1
	})):Play();
	(__lt.cm("TweenService", "Create", container, TweenInfo.new(0.26, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
		Size = UDim2.new(0, 520, 0, 380)
	})):Play();
	(__lt.cm("TweenService", "Create", panel, TweenInfo.new(0.26, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		BackgroundTransparency = 0.04
	})):Play();
	(__lt.cm("TweenService", "Create", panelShadow, TweenInfo.new(0.26, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		ImageTransparency = 0.5
	})):Play();
...
do
	...
	trackConn(UserInputService.InputChanged:Connect(function(inp)
		if dragging and inp == dragInput then
			...
			(__lt.cm("TweenService", "Create", container, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				Position = target
			})):Play();
			if minimized then
				dockButton.Position = target;
			end;
		end;
	end));
end;
do
	...
	dockButton.InputChanged:Connect(function(inp)
		if inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch then
			dragInput = inp;
		end;
	end);
...
(__lt.cm("TweenService", "Create", container, TweenInfo.new(0.28, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
	Size = UDim2.new(0, 520, 0, 380)
})):Play();
local hue = 0;
...
local function validTarget(p)
	...
	if ignoreFriends and isFriend(p.UserId) then
		return false;
	end;
...
local function fireOnPlayer(p)
	...
	for i = 1, #attackRemotes do
		attackRemotes[i]:FireServer(tp);
	end;
end;
...
trackConn(RunService.Heartbeat:Connect(function(dt)
	accumulator += dt;
	if accumulator < checkInterval or (not rootPart) then
		return;
	end;
	accumulator = 0;
	...
	if auraEnabled and aura then
		...
		if (aura.Size - goal).Magnitude > 0.01 then
			(__lt.cm("TweenService", "Create", aura, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
				Size = goal
			})):Play();
		end;
	end;
