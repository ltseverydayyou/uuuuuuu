...
local Themes = Import(script.Parent.Parent.Features.Themes);
local PADDING = 0;
return function(icon)
	...
	icon.dropdownChildAdded:Connect(function(childIcon)
		local _, modificationUID = childIcon:modifyTheme({
			...
			},
			{
				"Selection",
				"Size",
				UDim2.new(1, -PADDING, 1, -PADDING)
			},
			{
				"Selection",
				"Position",
				UDim2.new(0, PADDING / 2, 0, PADDING / 2)
			}
		});
		task.defer(function()
	...
	local function updateVisibility()
		...
		if icon.isSelected then
			...
			openTween = __lt.cm("TweenService", "Create", dropdown, tweenInfo, {
				Size = UDim2.new(0, dropdown.Size.X.Offset, 0, height)
			});
			openTween:Play();
			...
			closeTween = __lt.cm("TweenService", "Create", dropdown, closeTweenInfo, {
				Size = UDim2.new(0, dropdown.Size.X.Offset, 0, 0)
			});
			closeTween:Play();
