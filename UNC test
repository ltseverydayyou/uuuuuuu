...
local IsOnMobile = (function()
	...
	if platform == Enum.Platform.None then
		return (game:GetService("UserInputService")).TouchEnabled and (not ((game:GetService("UserInputService")).KeyboardEnabled or (game:GetService("UserInputService")).MouseEnabled));
	end;
	return false;
end)();
...
test("hookfunction", {
	...
	print("UNC_HOOKFUNCTION_TEST");
	assert(called == true, "hookfunction did not intercept target function");
end);
...
test("run_on_actor", {
	...
	flag.Value = false;
	flag.Parent = rp;
	local src = "local rp = game:GetService(\"ReplicatedStorage\")\nlocal v = rp:FindFirstChild(\"UNC_ACTOR_TEST\")\nif v then\n\tv.Value = true\nend\n";
