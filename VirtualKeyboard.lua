...
local function ClonedService(n)
	local S = function(_, serviceName) return __lt.gs(serviceName); end;local R = cloneref or function(x)
		return x
	end
	return __lt.cs(n, R)
end
...
local parentUI = (gethui and gethui()) or coreGui or (lp and lp:FindFirstChildWhichIsA("PlayerGui"))
if not parentUI then
	parentUI = game
end

local old = parentUI:FindFirstChild("VKB")
if old then
	old:Destroy()
end
...
tbPad.PaddingRight = UDim.new(0, 210)
tbPad.Parent = titleBar
...
trLayout.Parent = topRight

local function newTopBtn(name, w, text, bg)
...
local procBtn = newTopBtn("Processed", 96, "Unprocessed", themes.Dark.Btn)
local modeBtn = newTopBtn("Mode", 88, "QWERTY", themes.Dark.Btn)
local sizeBtn = newTopBtn("Size", 80, "Comfort", themes.Dark.Btn)
local colorBtn = newTopBtn("Palette", 80, "Palette", themes.Dark.Btn)
local rgbBtn = newTopBtn("RGB", 80, "Rainbow", shade(themes.Dark.Acc, 0.2))
local themeBtn = newTopBtn("Theme", 70, "Dark", themes.Dark.Btn)
local addBtn = newTopBtn("Add", 40, "+", themes.Dark.Acc)
local closeBtn = newTopBtn("Close", 38, "X", Color3.fromRGB(200, 60, 60))
...
local function kcFromLabel(t)
	local r = {
		...
		LS = Enum.KeyCode.ButtonL3,
		RS = Enum.KeyCode.ButtonR3
	}
	if r[t] then
		return r[t]
	end
...
local function pressConn(btn, kc)
	...
	btn.MouseButton1Down:Connect(function()
		...
		if kc and (not selectMode) and vim and vim.SendKeyEvent then
			__lt.cm("VirtualInputManager", "SendKeyEvent", true, kc, sendProcessed, game)
		end
	end)
	btn.MouseButton1Up:Connect(function()
		...
		if kc and (not selectMode) and vim and vim.SendKeyEvent then
			__lt.cm("VirtualInputManager", "SendKeyEvent", false, kc, sendProcessed, game)
		end
	end)
...
local function makeFloatKey(lbl, kc)
	...
	keyBtn.ZIndex = 101
	keyBtn.Parent = wrap
	local tsz = Instance.new("UITextSizeConstraint")
	tsz.MaxTextSize = 28
	tsz.MinTextSize = 12
	tsz.Parent = keyBtn
	...
	close.ZIndex = 103
	close.Parent = wrap
	local cts = Instance.new("UITextSizeConstraint")
	cts.MaxTextSize = 16
	cts.MinTextSize = 10
	cts.Parent = close
...
local function makeKey(t)
	...
	b.MouseButton1Click:Connect(function()
		if selectMode then
			makeFloatKey(t, kc)
			selectMode = false
			addBtn.BackgroundColor3 = rainbowOn and Color3.fromHSV(hue, 0.85, 1) or (themes[curTheme] and themes[curTheme].Acc or themes.Dark.Acc)
		end
	end)
...
local function layQWERTY()
	clearKeys()
	local rows = 6
	local baseH = math.max(28, math.floor((keysScroll.AbsoluteSize.Y - (rows - 1) * 8) / rows))
	local h = math.floor(baseH * rowScale)
	local r1 = newRow(h, 1)
	addWeighted(r1, {
		{ "Esc", 1 },
		"F1",
...
procBtn.MouseButton1Click:Connect(function()
	sendProcessed = not sendProcessed
	procBtn.Text = sendProcessed and "Processed" or "Unprocessed"
	refreshTheme()
end)

sizeBtn.MouseButton1Click:Connect(function()
	local order = { "Compact", "Comfort", "Large" }
	local i = table.find(order, sizeMode) or 2
...
addBtn.MouseButton1Click:Connect(function()
	...
	local acc = rainbowOn and Color3.fromHSV(hue, 0.85, 1) or (themes[curTheme] and themes[curTheme].Acc or themes.Dark.Acc)
	addBtn.BackgroundColor3 = selectMode and Color3.fromRGB(180, 60, 60) or acc
end)
...
local namedColors = {
	...
	{ "Orange", Color3.fromRGB(245, 140, 60) },
	{ "Amber", Color3.fromRGB(240, 180, 70) },
	{ "Yellow", Color3.fromRGB(250, 220, 90) },
	{ "Lime", Color3.fromRGB(170, 220, 60) },
	{ "Green", Color3.fromRGB(60, 200, 120) },
	{ "Teal", Color3.fromRGB(50, 200, 200) },
	{ "Cyan", Color3.fromRGB(60, 200, 250) },
	{ "Sky", Color3.fromRGB(90, 170, 255) },
	{ "Blue", Color3.fromRGB(90, 130, 255) },
	{ "Indigo", Color3.fromRGB(100, 110, 255) },
	{ "Violet", Color3.fromRGB(150, 100, 255) },
	{ "Purple", Color3.fromRGB(180, 100, 255) },
	{ "Magenta", Color3.fromRGB(220, 90, 220) },
	{ "Pink", Color3.fromRGB(255, 120, 160) },
	{ "Brown", Color3.fromRGB(150, 100, 70) },
	{ "Slate", Color3.fromRGB(110, 120, 140) },
	{ "Silver", Color3.fromRGB(210, 210, 220) },
...
for _, pair in ipairs(namedColors) do
	...
	sw.Parent = palette
	local sct = Instance.new("UITextSizeConstraint")
	sct.MaxTextSize = 14
	sct.MinTextSize = 10
	sct.Parent = sw
...
workspace:GetPropertyChangedSignal("CurrentCamera"):Connect(function()
	...
	if c then
		c:GetPropertyChangedSignal("ViewportSize"):Connect(updateLayout)
	end
end)
