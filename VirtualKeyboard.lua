...
mainFrm.Parent = ui
local c1 = Instance.new("UICorner") c1.CornerRadius = UDim.new(0, 12) c1.Parent = mainFrm
...
local c2 = Instance.new("UICorner") c2.CornerRadius = UDim.new(0, 12) c2.Parent = titleBar
...
local tlc = Instance.new("UITextSizeConstraint", titleLbl) tlc.MaxTextSize = 16 tlc.MinTextSize = 10
...
local function newTopBtn(name, xoff, w, text, bg)
    ...
    b.TextScaled = true
    local cc = Instance.new("UITextSizeConstraint", b) cc.MaxTextSize = 14 cc.MinTextSize = 10
    b.Parent = titleBar
...
local tc = Instance.new("UITextSizeConstraint", toggleBtn) tc.MaxTextSize = 18 tc.MinTextSize = 12
toggleBtn.Parent = ui
local c6 = Instance.new("UICorner") c6.CornerRadius = UDim.new(0, 12) c6.Parent = toggleBtn
...
local function kcFromLabel(t)
    local r = {
        ...
        ["A"]=Enum.KeyCode.ButtonA,["B"]=Enum.KeyCode.ButtonB,["X"]=Enum.KeyCode.ButtonX,["Y"]=Enum.KeyCode.ButtonY,
...
local function pressConn(btn, kc)
    ...
    btn.MouseButton1Down:Connect(function()
        ...
        down = true
        if kc and not selectMode then vim:SendKeyEvent(true, kc, false, game) end
    end)
...
local function makeDrag(obj, handle)
    ...
    handle.InputBegan:Connect(function(input)
        if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
            dragging=true start=input.Position startPos=obj.Position
...
local function makeFloatKey(lbl, kc)
    ...
    local tsz = Instance.new("UITextSizeConstraint", keyBtn) tsz.MaxTextSize = 28 tsz.MinTextSize = 12
    keyBtn.Font = Enum.Font.SourceSansBold
    keyBtn.BorderSizePixel = 0
    keyBtn.ZIndex = 101
    keyBtn.Parent = wrap
    local ic = Instance.new("UICorner") ic.CornerRadius = UDim.new(0.4, 0) ic.Parent = keyBtn
    local st = Instance.new("UIStroke") st.Thickness = 2 st.Color = Color3.fromRGB(0,0,0) st.Transparency = 0.45 st.Parent = keyBtn
    ...
    local cts = Instance.new("UITextSizeConstraint", close) cts.MaxTextSize = 16 cts.MinTextSize = 10
...
local function makeKey(t)
    ...
    b.Text = t
    b.TextScaled = true
    local cst = Instance.new("UITextSizeConstraint", b) cst.MaxTextSize = 18 cst.MinTextSize = 10
    b.Font = Enum.Font.SourceSans
    b.AutoButtonColor = true
    b.BorderSizePixel = 0
    local cor = Instance.new("UICorner") cor.CornerRadius = UDim.new(0, 8) cor.Parent = b
...
local function spacer(w) local f = Instance.new("Frame") f.BackgroundTransparency=1 return {f, w or 1} end

local function clearKeys()
    activeKeys = {}
    for _,c in ipairs(keysScroll:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
end
...
local function stopRainbow() if rainbowConn then rainbowConn:Disconnect() rainbowConn=nil end refreshTheme() end
...
local function makeDragBtn(f) makeDrag(f, f) end
...
closeBtn.MouseButton1Click:Connect(function() if rainbowConn then rainbowConn:Disconnect() end ui:Destroy() end)
...
local pc = Instance.new("UICorner") pc.CornerRadius = UDim.new(0, 10) pc.Parent = palette
...
for _,pair in ipairs(namedColors) do
    ...
    sw.Text = n
    sw.TextScaled = true
    local sct = Instance.new("UITextSizeConstraint", sw) sct.MaxTextSize = 14 sct.MinTextSize = 10
    ...
    sw.MouseButton1Click:Connect(function() applyBaseColor(c) palette.Visible=false end)
end

colorBtn.MouseButton1Click:Connect(function() palette.Visible = not palette.Visible end)
