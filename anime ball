...
local function __NAProtectUI(gui, options)
	...
	return nil;
			...
			return __lt.cv(value)
		end,
		__newindex = function(_, key, value)
			__lt.gs(name)[key] = value
		end,
		__tostring = function()
			return tostring(ref)
		end
...
if game.GameId == 5120885191 then
	local function ClonedService(name)
		local Service = function(_, serviceName) return __lt.gs(serviceName); end;local Reference = cloneref or function(reference)
			return reference
		end
	...
	local function attachAntiKb(char)
		...
		for _, desc in ipairs(char:QueryDescendants("Instance")) do
	...
	local function disconnectAntiFx()
		if antiFxConn then
			antiFxConn:Disconnect()
			antiFxConn = nil
		end
	end

	local function purgeEffects()
		for _, desc in ipairs(__lt.cm("Workspace", "QueryDescendants", "Instance")) do
			if isEffectInstance(desc) then
				desc:Destroy()
			end
		end
	end

	local function attachAntiFx()
		disconnectAntiFx()
		if not antiFx then
			return
		end
		task.spawn(purgeEffects)
		antiFxConn = wk.DescendantAdded:Connect(function(desc)
			task.defer(function()
				if isEffectInstance(desc) then
					desc:Destroy()
				end
	...
	local function pulseBtn(btn)
		...
		local tweenIn = __lt.cm("TweenService", "Create", btn, TweenInfo.new(0.06, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
		...
		task.delay(0.1, function()
			if btn and btn.Parent then
				__lt.cm("TweenService", "Create", btn, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
					BackgroundColor3 = bg,
					TextColor3 = tc
				}):Play()
			end
	...
	local function updateStatus()
		...
		statusLbl.Text = "Spam: " .. (spam and "ON" or "OFF") .. "   Speed: " .. spamMode .. "   Anti KB: " .. (antiKb and "ON" or "OFF") .. "   Anti FX: " .. (antiFx and "ON" or "OFF") .. "   Device: " .. dev
		if UIS.TouchEnabled then
			hintLbl.Text = "Tap buttons to toggle spam, speed, anti knockback, anti effects and freeze."
		else
			hintLbl.Text = "Press R to toggle spam. Use buttons for speed, anti knockback, anti effects and freeze."
		end
	end

	local function tween(obj, t, goal)
		local tw = __lt.cm("TweenService", "Create", obj, TweenInfo.new(t, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), goal)
		tw:Play()
		return tw
	end
	...
	local function setAntiFx(state)
		antiFx = state
		if antiFxBtn then
			antiFxBtn.Text = "Anti Effects: " .. (antiFx and "ON" or "OFF")
			setBtnState(antiFxBtn, antiFx)
		end
		if antiFx then
			attachAntiFx()
		else
	...
	local function toggleAntiFx()
		setAntiFx(not antiFx)
	end
	...
	antiKbBtn = mkBtn("AntiKnockbackBtn", "Anti Knockback: OFF", UDim2.new(0, 150, 0, 34), UDim2.new(0, 14, 0, 158), function()
		toggleAntiKb()
	end, false)
	antiFxBtn = mkBtn("AntiEffectsBtn", "Anti Effects: OFF", UDim2.new(0, 150, 0, 34), UDim2.new(0, 176, 0, 158), function()
		toggleAntiFx()
	end, false)
	...
	function handleBall(ball)
		...
		for _, v in ipairs(ball:QueryDescendants("Instance")) do
