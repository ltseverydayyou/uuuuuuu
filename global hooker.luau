...
if getgc and hookfunc and getrenv and debugInfo then
	...
	for _, v in getgc(true) do
		if typeof(v) == "table" then
			...
			if typeof(det) == "function" or typeof(kill) == "function" and vars and proc then
				...
				if typeof(kill) == "function" then
					KillMeth = kill;
					hookfunc(KillMeth, function(killFunc)
					end);
				end;
			end;
		end;
	...
	if DetectedMeth and debugInfo then
		...
		hookfunc(debugInfo, newcclosure(function(...)
			local name = ...;
...
local oldIndex, oldNewIndex, oldNamecall = rawmeta.__index, rawmeta.__newindex, rawmeta.__namecall;
...
rawmeta.__newindex = newcclosure(function(obj, key, val)
	...
	if propertyTypes[obj] and propertyTypes[obj][key] then
		...
		if typeof(val) ~= t then
			error("bad argument #3 to '" .. key .. "' (" .. t .. " expected, got " .. typeof(val) .. ")");
		end;
...
rawmeta.__namecall = newcclosure(function(obj, ...)
	...
	if callHooks[obj] and callHooks[obj][method] then
		return callHooks[obj][method].Callback(oldIndex(obj, method), table.unpack(args));
	end;
	if globalCallHooks[method] then
		return globalCallHooks[method].Callback(obj, oldIndex(obj, method), table.unpack(args));
	end;
