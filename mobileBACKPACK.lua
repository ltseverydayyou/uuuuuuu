...
local bar=Instance.new("Frame") bar.Name="Bar" bar.AnchorPoint=Vector2.new(.5,1) bar.Position=UDim2.new(.5,0,1,-gap) bar.Size=UDim2.new(0,qbW+(mob and 220 or 260),0,barH) bar.BackgroundTransparency=1 bar.Parent=ui
local row=Instance.new("Frame") row.AnchorPoint=Vector2.new(.5,.5) row.Position=UDim2.new(.5,0,.5,0) row.Size=UDim2.new(1,0,1,0) row.BackgroundTransparency=1 row.Parent=bar
...
local rDock=Instance.new("Frame") rDock.Name="RD" rDock.Size=UDim2.new(0, bW*3 + (mob and 8 or 8)*2 ,0,bH) rDock.BackgroundTransparency=1 rDock.Parent=row
...
local expFr=Instance.new("Frame") expFr.Name="Exp" expFr.Size=UDim2.new(0,qbW,0,0) expFr.AnchorPoint=Vector2.new(.5,1) expFr.Position=UDim2.new(.5,0,1,-(barH+gap+8)) expFr.BackgroundColor3=Thm.bg expFr.BackgroundTransparency=.2 expFr.BorderSizePixel=0 expFr.Visible=true expFr.ClipsDescendants=true expFr.Parent=ui corner(expFr,12) stroke(expFr,1)
...
local cfm=Instance.new("Frame") cfm.Name="Cfm" cfm.AnchorPoint=Vector2.new(.5,.5) cfm.Size=UDim2.new(0,300,0,78) cfm.Position=UDim2.new(.5,0,.5,0) cfm.BackgroundColor3=Thm.soft cfm.BackgroundTransparency=1 cfm.Visible=false cfm.ZIndex=800 cfm.Parent=ui corner(cfm,10) stroke(cfm,1)
...
local cYes=Instance.new("TextButton") cYes.Size=UDim2.new(0,56,0,36) cYes.Position=UDim2.new(1,-124,.5,-18) cYes.Text="Yes" cYes.Font=Enum.Font.GothamBold cYes.TextScaled=true cYes.BackgroundColor3=Thm.dng cYes.TextColor3=Thm.txt cYes.TextTransparency=1 cYes.ZIndex=801 cYes.Parent=cfm corner(cYes,8) stroke(cYes,1)
local cNo=Instance.new("TextButton") cNo.Size=UDim2.new(0,56,0,36) cNo.Position=UDim2.new(1,-62,.5,-18) cNo.Text="No" cNo.Font=Enum.Font.GothamBold cNo.TextScaled=true cNo.BackgroundColor3=Thm.soft cNo.TextColor3=Thm.txt cNo.TextTransparency=1 cNo.ZIndex=801 cNo.Parent=cfm corner(cNo,8) stroke(cNo,1)

local tip=Instance.new("Frame") tip.Name="Tip" tip.BackgroundColor3=Thm.soft tip.BackgroundTransparency=.05 tip.Visible=false tip.ZIndex=700 tip.Parent=ui corner(tip,8) stroke(tip,1)
...
local mTop=Instance.new("Frame") mTop.Name="MTop" mTop.AnchorPoint=Vector2.new(.5,0) mTop.Size=UDim2.new(0, mob and 170 or 0, 0, mob and 36 or 0) mTop.Position=UDim2.new(.5,0,0,(mob and 8 or 0)) mTop.BackgroundColor3=Thm.soft mTop.BackgroundTransparency=.1 mTop.Visible=false mTop.ZIndex=1000 mTop.Parent=ui corner(mTop,10) stroke(mTop,1)
...
local function mBtn(txt,col) local b=Instance.new("TextButton") b.Size=UDim2.new(0,50,0,30) b.Text=txt b.TextScaled=true b.Font=Enum.Font.GothamBold b.BackgroundColor3=col or Thm.soft b.TextColor3=Thm.txt b.ZIndex=1001 b.Parent=mTop corner(b,8) stroke(b,1) return b end
local mFav=mBtn("★") local mLock=mBtn("🔓") local mDrop=mBtn("D",Thm.dng)

local mini=Instance.new("TextButton") mini.Name="Mini" mini.AnchorPoint=Vector2.new(.5,1) mini.Position=UDim2.new(.5,0,1,-gap) mini.Size=UDim2.new(0,(mob and 64 or 72),0,(mob and 32 or 36)) mini.Text="Open" mini.Font=Enum.Font.GothamBold mini.TextScaled=true mini.BackgroundColor3=Thm.soft mini.BackgroundTransparency=.1 mini.TextColor3=Thm.txt mini.Visible=false mini.ZIndex=999999 mini.Parent=ui corner(mini,10) stroke(mini,1)
...
local function orderedQBButtons() local arr={} for _,ch in ipairs(qb:GetChildren()) do if ch:IsA("GuiButton") then table.insert(arr,ch) end end table.sort(arr,function(a,b) return (a.LayoutOrder or 0) < (b.LayoutOrder or 0) end) return arr end
...
local function tog(t)
	...
	if t.Parent==c then
		...
		task.wait()
		if t.Parent==c then t.Parent=bp end
		sel=nil
	else
		pcall(function() h:UnequipTools() end)
...
local function dropOne(t) local c=Plr.Character if not c then return end if t.Parent==c then t.Parent=WS else t.Parent=c task.wait() t.Parent=WS end if sel==t then sel=nil updMobileTop() end end
...
local function opsBar(b,t)
	...
	local function miniB(txt) local x=Instance.new("TextButton") x.Size=UDim2.new(0,18,0,12) x.Text=txt x.Font=Enum.Font.GothamBold x.TextScaled=true x.BackgroundColor3=Thm.soft x.BackgroundTransparency=.1 x.TextColor3=Thm.txt x.BorderSizePixel=0 x.ZIndex=8 x.Parent=ops corner(x,4) return x end
	local fav=miniB("★") local lock=miniB(getattr(t,"BP_Lock",false) and "🔒" or "🔓") local drop=nil if t.CanBeDropped then drop=miniB("D") end
	fav.TextColor3=getattr(t,"BP_Fav",false) and Thm.yel or Thm.txt
	fav.MouseButton1Click:Connect(function() local nv=not getattr(t,"BP_Fav",false) attr(t,"BP_Fav",nv) need=false rebuild() end)
	lock.MouseButton1Click:Connect(function() local nv=not getattr(t,"BP_Lock",false) attr(t,"BP_Lock",nv) need=false rebuild() end)
...
mFav.MouseButton1Click:Connect(function() if not sel or not sel:IsA("Tool") then return end attr(sel,"BP_Fav", not getattr(sel,"BP_Fav",false)) rebuild() end)
mLock.MouseButton1Click:Connect(function() if not sel or not sel:IsA("Tool") then return end attr(sel,"BP_Lock", not getattr(sel,"BP_Lock",false)) rebuild() end)
mDrop.MouseButton1Click:Connect(function() if not sel or not sel:IsA("Tool") then return end if canDrop(sel) then dropOne(sel) rebuild() end end)

local function place() bar.Position=UDim2.new(.5,0,1,-gap) expFr.Position=UDim2.new(.5,0,1,-(barH+gap+8)) end
...
function rebuild()
	...
	if sel and not map[sel] then sel=nil end
	...
	table.sort(list,function(a,b) local fa=getattr(a,"BP_Fav",false) and 0 or 1 local fb=getattr(b,"BP_Fav",false) and 0 or 1 if fa~=fb then return fa<fb end return (idxMap[a] or 1e9)<(idxMap[b] or 1e9) end)
	...
	local function mkBtn(t,i)
		...
		tBtns[b]=t
		if not mob and i<=10 then local f=Instance.new("Frame") f.Name="Key" f.Size=UDim2.new(0,20,0,20) f.Position=UDim2.new(0,3,0,3) f.BackgroundColor3=Thm.bg f.BackgroundTransparency=.1 f.ZIndex=9 f.Parent=b corner(f,5) stroke(f,1) local tX=Instance.new("TextLabel") tX.BackgroundTransparency=1 tX.Size=UDim2.new(1,0,1,0) tX.Font=Enum.Font.GothamBold tX.TextScaled=true tX.TextColor3=Thm.txt tX.Text=(i==10 and "0" or tostring(i)) tX.ZIndex=10 tX.Parent=f local ts=Instance.new("UITextSizeConstraint") ts.MaxTextSize=14 ts.MinTextSize=10 ts.Parent=tX end
		opsBar(b,t)
		if not mob then
	...
	for i,t in ipairs(list) do local b=mkBtn(t,i) b.LayoutOrder=i if i<=10 then b.Parent=qb else b.Parent=scr end end
	local extra=math.max(#list-10,0) local rows=math.ceil(extra/10) scr.CanvasSize=UDim2.new(0,0,0,rows*(cell+pad))

	local d=cntDrop() drpBtn.AutoButtonColor=d>0 drpBtn.Active=d>0 drpBtn.TextTransparency=d>0 and 0 or .4 drpBtn.BackgroundTransparency=d>0 and .05 or .5 drpBtn.Text="Drop("..tostring(d)..")"
	...
	if col then bar.Visible=false exp=false expFr.Visible=false setStrokeOn(false) mini.Visible=true mini.Text="Open" else bar.Visible=true expFr.Visible=true setStrokeOn(true) mini.Visible=false end
...
local function showBar() bar.Visible=true bar.Position=UDim2.new(.5,0,1,barH+40) exp=false expFr.Visible=false setStrokeOn(false) tw(bar,.22,{Position=UDim2.new(.5,0,1,-gap)}) task.wait(.02) end
local function hideBar() tw(bar,.18,{Position=UDim2.new(.5,0,1,barH+40)}) task.delay(.18,function() if col then bar.Visible=false end end) end

local function toggleCol() col=not col if col then hideBar() tw(expFr,.16,{Size=UDim2.new(0,qbW,0,0)}) setStrokeOn(false) mini.Visible=true mini.Text="Open" else showBar() mini.Visible=false setStrokeOn(true) end end
local function toggleExp() if col then return end exp=not exp req() end

local running=false
local function doDropAll() local d=cntDrop() if d<=0 or running then return end showCfm() end
...
cYes.MouseButton1Click:Connect(function()
	...
	local h=hum() if h then pcall(function() h:UnequipTools() end) end
	local c=Plr.Character for _,t in ipairs(lst) do if t and t.Parent and t.Parent~=WS then if c and t.Parent~=c then t.Parent=c task.wait() end t.Parent=WS end end
	sel=nil updMobileTop() hideCfm() running=false req()
end)
