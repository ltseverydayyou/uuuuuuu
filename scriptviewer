...
local cSource, cName	= "", ""
local localPlayer		= ClonedService("Players").LocalPlayer
...
function getTextSize(label)
	local service = ClonedService("TextService")
	local vec2 = __lt.cm("TextService", "GetTextSize", label.Text, label.TextSize, label.Font, Vector2.new(10000, 25))
	return vec2
end
...
function getScriptsOfParent(p)
	local list = {}
	local objs = (p == nil and getnilinstances and getnilinstances()) or p:QueryDescendants("Instance")
	if p == game then objs = (getscripts and getscripts()) or game:QueryDescendants("Instance") end
...
createButton(scriptList,
	{Name="LocalPlayer", Type="Folder", Obj=ClonedService("Players").LocalPlayer}
)
