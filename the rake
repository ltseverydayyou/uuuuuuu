...
if game.GameId == 847722000 then
	...
	function clientBypass.bindFlashlightObj(obj)
		...
		local props = {}
		if obj:IsA("Light") then
			props = { "Enabled", "Brightness", "Range", "Shadows" }
			if obj:IsA("SpotLight") then
				props[#props + 1] = "Angle"
			end
		elseif obj:IsA("Beam") then
			props = { "Enabled", "Width0", "Width1", "Brightness", "LightEmission", "Transparency" }
		elseif obj:IsA("ParticleEmitter") then
			props = { "Enabled", "Rate", "LightEmission", "ZOffset" }
		else
			return
		end
	...
	ExploitsTab:CreateButton({
		...
		Callback = function()
			...
			if n <= 0 then
				itemNote("Bring Scraps", "No scraps found. Wait until they spawn in.")
			else
				itemNote("Bring Scraps", "Brought " .. tostring(n) .. " scrap" .. (n == 1 and "." or "s."))
			end
		end,
	})
	...
	ExploitsTab:CreateButton({
		...
		Callback = function()
			...
			if n <= 0 then
				itemNote("Bring Flaregun", "No flaregun found. Wait until it spawns in.")
			else
				itemNote("Bring Flaregun", "Brought flaregun.")
			end
		end,
	})
