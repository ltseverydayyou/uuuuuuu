...
local function SafeGetService(name)
	local GetService = function(_, serviceName) return __betterGetService(serviceName) end;
...
local function protectUI(gui)
	local protected = __NAProtectUI(gui)
	if protected then
		return protected
	end
	...
	if gethui then
		target = gethui();
	elseif CoreGui and __lt.cm("CoreGui", "FindFirstChild", "RobloxGui") then
		target = __lt.cm("CoreGui", "FindFirstChild", "RobloxGui");
	elseif CoreGui then
		target = CoreGui;
	elseif player then
...
local function t(i, time, style, dir, props)
	(__lt.cm("TweenService", "Create", i, TweenInfo.new(time, style or Enum.EasingStyle.Quad, dir or Enum.EasingDirection.Out), props)):Play();
end;
...
local function setSliderFromInput(input)
	local xPos = input and input.Position and input.Position.X or (__lt.cm("UserInputService", "GetMouseLocation")).X;
