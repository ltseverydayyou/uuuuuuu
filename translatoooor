	...
	for k, v in pairs(data) do
		if type(v) == "table" then
			for _, vv in pairs(v) do
				s ..= "&" .. __lt.cm("HttpService", "UrlEncode", k) .. "=" .. __lt.cm("HttpService", "UrlEncode", vv);
			end;
		else
			s ..= "&" .. __lt.cm("HttpService", "UrlEncode", k) .. "=" .. __lt.cm("HttpService", "UrlEncode", v);
		end;
	end;
	return s:sub(2);
end;
local jE = function(x)
	return __lt.cm("HttpService", "JSONEncode", x);
end;
local jD = function(x)
	return __lt.cm("HttpService", "JSONDecode", x);
end;
local rpc = "MkEWBc";
...
local function sys(msg)
	local chans = __lt.cm("TextChatService", "WaitForChild", "TextChannels");
...
task.spawn(function()
	repeat
		task.wait();
	until __lt.cm("CoreGui", "FindFirstChild", "ExperienceChat");
	local ec = __lt.cm("CoreGui", "WaitForChild", "ExperienceChat");
...
TCS.MessageReceived:Connect(function(msg)
	...
	local p = __lt.cm("Players", "GetPlayerByUserId", uid);
